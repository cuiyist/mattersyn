...
Include selected original characterization figures only when the exact asset has a recorded redistribution basis and required attribution. Keep readable scales/axes, short explanations and enlarged views. Where permission or licensing remains unresolved, retain measured facts and source locators and provide an explicit source link or an independently authored illustration; never imply that an unavailable original TEM image or spectrum is displayed. Distinguish measured data, the paper's simulations and the website's illustrative models. Missing techniques remain absent or explicitly unreported in the inspected sources; a requested example such as Raman does not justify inventing a spectrum. Sources, unresolved details, structured downloads and preserved examples belong in the appendix.
...
- Publish through the user's selected hosting provider, preserving project identity and the requested audience. The latest MatterSyn instruction makes both `cuiyist/mattersyn` (project, memory, skills and history) and `cuiyist/mattersyn-site` (GitHub Pages website) public, while original papers/SI remain local. Publish saved progress and references at meaningful milestones; follow the source-exclusion and citation rules in the delivery reference. An invocation does not authorize unrelated external messages or a different audience.
