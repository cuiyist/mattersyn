...
- Publish through the user's selected hosting provider, preserving project identity and the requested audience. The latest MatterSyn instruction makes `cuiyist/mattersyn` private (code, reviewed data, memory, skills and cleaned history) while keeping `cuiyist/mattersyn-site` and its GitHub Pages website public. Complete papers/SI, raw caches, unreviewed candidates and private evidence remain local. Save project progress privately and publish reviewed website contributions and citations at meaningful milestones. Follow the delivery reference; do not change visibility again or send an external support request without user instruction.
