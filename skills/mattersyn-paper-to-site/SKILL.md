...
Read the current `research-assets/curation-control.json` before scheduled curation. A newer explicit user resume can supersede a historical trial-end hold; retain the completed trial's counts and receipts unchanged and measure resumed work in a separate run. Scheduled reports never override a newer pause. Verify the current saved paper version before reusing a progress description: if candidate counts or hashes differ, reconcile the versions and report the uncertainty before acceptance.

Reduce mechanical integration work with immutable accepted packages, early dependency/hash checks and additive imports into a private staging tree. Reject stale bindings, duplicate source/record IDs and unreviewed dependencies before touching shared data. Preserve exact accepted record/review bytes and independently review the actual shared integration. Measure build stages separately from scientific review; faster draft or build output is not evidence of higher completed-paper throughput.
...
## Throughput: complete-package preflight

Before handing a paper to the integration owner, freeze one authoritative schema-valid package and a coverage map. Verify every Reader/record context link targets an included public-safe file or an existing reviewed page; private extraction drafts are not public link targets. Bind every material occurrence, including source-named mixtures whose ingredients remain unknown, and distinguish an unresolved identity binding from an explicitly bound mixture with unknown composition. Close review-status metadata against exact passed receipts and rebind dependent hashes without changing scientific fields. Include stock components, aliquot distinctions, source-figure/sample locators, presentation deltas, and exact base hashes in the same handoff. Check illustration text bounds at its intended size.
