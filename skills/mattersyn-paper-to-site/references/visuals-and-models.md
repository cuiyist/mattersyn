...
6. Highlight actual selected functional-group atoms/bonds. Validate each group chemically using neighbors, bond orders and formal charges, rather than atom degree alone: a three-coordinate nitrate nitrogen is not a tertiary amine. A correction to highlight metadata must preserve already verified connectivity and geometry and receive its own bounded audit. Keep molecular formulas and roles visible. Distinguish a free ligand from a surface-bound ligand, a named stock precursor from unresolved stock speciation, and a solid-element reference from its dissolved form.
...
### Cross-paper integration checks from the local throughput trial

- Bind a morphology interpretation to the resulting product context, not the fractionation feed. Switching specimens must remove an interpretation that lacks evidence for the newly selected context.
- Preserve units, approximation signs, bounds and the definition of a reported dispersion in both canonical measurements and Reader fact cards. Do not infer an omitted unit from a familiar scientific symbol.
- When a scheme and experimental paragraph disagree, cite each value to the passage that actually supports it. A broad operation-level citation may retain both sources, but the individual parameter must not cite the opposing value as support. Expose the unresolved conflict in the recipe and paper review.
- A typical-method vessel capacity or filling limit does not automatically apply to every figure-caption variant. Keep inherited general context distinct from variant-specific reported conditions; label apparatus geometry and liquid levels as conceptual.
- Route admission requires an actual reviewed synthesis record. An observation record must not become a synthesis method because it carries a legacy task tag; an explicit contextual Reader role overrides legacy fallbacks.
- Metadata closeout can change record hashes without changing scientific values. Refresh current figure, molecule, stock, product and morphology bindings, while preserving the original audit receipts and their frozen input hashes.
...
- For destructive analytical preparation, identify the original preparation separately from the measured state. A diffraction phase of the source nanocrystal is not the phase or complete formula of its ICP acid digest. Likewise, film diffraction, dispersion luminescence and device aging need separate specimen contexts even when the paper uses a shared material name. Retain supplied coordinate tables and their conflicts without equating an unqualified reconstruction with absent structure information.
...
# Shared Reader integration checks

Bind stock-context IDs to both the record and stock identity. Reusing a stock name across routes does not establish the same composition, preparation or batch. Keep source-reported precursor products and their molecular reference drawings in precursor context, separate from nanocrystal specimen structures and coordinate labels.

Classify protocol conditions using the parameter meaning and unit together. A parameter such as `room_temperature_degas_duration` is a duration; its value must not appear as temperature. Keep cold addition, room-temperature reaction, subsequent heating and workup as separate visual phases when a source operation contains several actions. A room-temperature Grignard step must not acquire a heater from a generic reaction illustration.

Validate generated molecular SVGs as XML after adding titles/descriptions. Insert metadata inside the root SVG element, accounting for an XML declaration. Test apparatus assets on both material hubs and nested record pages; resolve shared assets from the site root rather than the current document directory. Exercise source-comparison cards with the actual incoming field names so claim text and locators remain visible.

- Before freezing a release, check authored molecular assets against both exact source and website delivery paths in the asset registry. A source-only path is insufficient for the site gate; preserve the already reviewed asset bytes and rights status when adding its site alias.
