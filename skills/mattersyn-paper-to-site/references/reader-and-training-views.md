...
## Rebuild persistence and complete route coverage

The every-material requirement includes future rebuilds. `scripts/build_reader_views.py` owns the common material page, CdSe hub and four legacy method aliases using `templates/material-reader.html`; do not restore the earlier material-hub generator. Preserve `*-evidence.html` editions. Run Reader metadata generation after the dataset and atlas builders. A broad change must compare the complete indexed hub list and canonical synthesis-route set against Reader metadata and entrypoints, then check all public URLs. Supporting procedures, observations and benchmark rows remain Data/evidence records rather than invented synthesis methods. Keep deployment checks distinct from independent scientific review.
