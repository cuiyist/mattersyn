...
Tirosh et al. (2006), DOI `10.1021/cm052401p`, is the first added full-main application of this pair rule. Method A at 270 °C has an explicit link to sample A and source-backed composition, ferrite phase, TEM size and XRD lattice parameter, so it contributes one broad recipe/sample structure row. The 230 and 250 °C variants have MCD temperature-series observations but no separately established phase, morphology or size assignment and therefore do not add rows. Methods B–D are retained as characterization comparisons because this paper delegates their synthesis procedures to other references. The cobalt-oxide FTIR control remains outside the CoFe2O4 routes. The bulk reference CIF is comparison-only; exact measured-coordinate and task-ready counts remain zero. Cross-paper physical-sample deduplication is still incomplete.
