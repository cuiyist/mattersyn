...
def material_reader_notes(material,record):
    ...
    return [n for n in dict.fromkeys(material['notes']) if not ((record['lineage']['source_group'],n) in {('lian2021','Chemical/atomic visual binding is separately pending.'),('ghosh2012','Source-qualified identity only; molecular/product visual binding is separately pending.')})]

def product_reader_note(note,record):
    prefix='Source context object: ' if record['lineage']['source_group']=='ghosh2012' else 'Source sample object: '
    if record['lineage']['source_group'] not in {'lian2021','ghosh2012'} or not note.startswith(prefix):return '<p>'+esc(note)+'</p>'
...
def render_record(r,meta):
    ...
    links=[dict(l,url=('../'+l['url'] if l['url'].startswith('paper-review.html') else record_href(l['url'])),relation=('Source document review' if (r['lineage']['source_group'],l['relation']) in {('evans2010','source_reader_pending_independent_audit'),('morrison2017','private_reader_pending_independent_review'),('lian2021','private_unapproved_reader_proposal'),('ghosh2012','private_unapproved_reader_proposal')} else l['relation'])) for l in r['context_links']]
