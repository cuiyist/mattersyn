...
def figbutton(src,caption):return '<button class="evidence-image-button" data-evidence-image="'+esc(src)+'" data-caption="'+esc(caption)+'"><img loading="lazy" src="'+esc(src)+'" alt="'+esc(caption)+'"><span>Enlarge original figure ↗</span></button>'
def main():
    ...
    (ROOT/'dist/murray-1993-characterization.html').write_text(shell('Murray 1993 figure archive',body,'evidence-gallery.mjs'),encoding='utf-8')
    n=a['related_paper_saed_check']['nakonechnyi2017'];s=n['saed'];src=s['original_figure_asset']['file']
    body='<nav class="material-hub-nav"><a href="index.html">Periodic table</a> → <a href="cdse.html">CdSe</a> → Core/shell SAED</nav><div class="dataset-heading"><span class="eyebrow">NAKONECHNYI ET AL. · 2017 · SUPPORTING INFORMATION S3</span><h1>Selected-area electron diffraction</h1><p>Original Figure S2: CdSe/CdS and CdSe/ZnSe core/shell particles.</p></div><p class="coverage-note">'+esc(s['scope_warning'])+'</p>'+figbutton(src,s['caption_paraphrase'])+'<div class="saed-panels">'+''.join('<article><h2>Panel '+esc(p['panel'])+'</h2><p>'+esc(p['product'])+'</p><p>Core: '+esc(p['core'])+'<br>Shell: '+esc(p['shell'])+'</p></article>' for p in s['panels'])+'</div><p>'+esc(s['sample_link_status'])+'</p><div class="download-links"><a href="https://doi.org/10.1021/acs.chemmater.7b00354">Primary paper ↗</a><a href="https://acs.figshare.com/doi/suppl/10.1021/acs.chemmater.7b00354">Supporting information ↗</a><a href="data/paper-evidence/nakonechnyi2017-saed.json" download>SAED evidence JSON ↓</a><a href="records/nakonechnyi-2017-zb-cdse-cds-seeded-growth.html">Reviewed CdSe/CdS growth record →</a></div>'+dialog()
    (ROOT/'dist/nakonechnyi-2017-saed.html').write_text(shell('Core/shell SAED',body,'evidence-gallery.mjs'),encoding='utf-8')
