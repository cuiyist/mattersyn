...
def synthesis_precursors(r):
    roles={'metal_precursor','host_precursor','dopant_precursor','chalcogen_precursor','halide_precursor','nonmetal_precursor','metalloid_precursor','surface_functionalization_reagent'}
...
def eligibility(r, structure_policy=None):
    ...
    result={k:{'eligible':bool(v[0]) and k in r['quality']['requested_tasks'],'reason':v[1] if base or k=='optical_outcome' else 'Source review required, or structured-data verification supports only a separately gated benchmark task.'} for k,v in candidates.items()}
...
def training_view(r,task,structure_policy=None):
