...
function sourceDisagreements(host,pairs,r){if(!Array.isArray(pairs)||!pairs.length)return;const heading=el('h3','Unresolved source comparisons','reader-conflict-heading');heading.id='source-disagreements';host.append(heading,el('p','Each account is retained with its own context and source locator. These reports have not been reconciled into a single value or sample assignment.','reader-conflict-intro'));for(const pair of pairs){if(!Array.isArray(pair.claims)||pair.claims.length<2)continue;const card=el('article',undefined,'reader-conflict');const top=el('div',undefined,'reader-conflict-top');top.append(el('h4',pair.topic||'Source disagreement'),badge('Unresolved','reader-conflict-status'));card.append(top);if(pair.context)card.append(el('p',pair.context,'reader-conflict-context'));const claims=el('div',undefined,'reader-conflict-claims');pair.claims.forEach((claim,index)=>{const item=el('section',undefined,'reader-conflict-claim');item.append(el('span',claim.label||`Report ${index+1}`,'reader-conflict-label'),el('p',claim.statement||'Claim text not supplied.'));if(claim.sourceLocator)item.append(el('small',claim.sourceLocator,'reader-conflict-locator'));const source=r.sources.find(s=>s.id===claim.sourceId);if(source)item.append(link(source.title+' ↗',sourceURL(source),'reader-data-link'));claims.append(item);});card.append(claims);host.append(card);}}
