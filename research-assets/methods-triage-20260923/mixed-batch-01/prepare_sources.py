...
for prefix in ('R_','U_'):
    ...
    for i in (0,len(pool)//4,len(pool)//2,3*len(pool)//4,len(pool)-1): selected.append(pool[i])
selection={'started_at':started,'author':'/root','selection_method':'five deterministic rank positions (endpoints/quartiles) in each eligible R/U band; purposive, not representative or random','ranker_sha256':sha(RUN/'ranked-scopes.jsonl'),'documents_sha256':sha(DOC),'scopes':selected}
