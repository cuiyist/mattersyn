...
for c,n in zip(d['inputs'],notes):
    ...
    receipts.append({**c,**n,'source_hashes':sorted(sourcehashes),'full_pdf_text_pages_read':readloc,'ooxml_text_read':xml,'original_pdf_pages_visually_inspected':[{'source_sha256':h,'pdf_page':pn}for h,pn in sorted(viewset)if h in sourcehashes],'identity_headers':'main/SI title-author headers inspected separately; not counted as full-page reading','independent_audit':'pending','full_extraction_complete':False,'training_admission':False})
for r in renders:assert sha(r['path'])==r['sha256']
summary={'started_at':s['started_at'],'ended_at':at.isoformat(),'author':'/root','status':'author_source_triage_complete_independent_check_pending','selected_scopes':10,'newly_triaged_scopes':9,'reused_existing_terminal_scope':1,'new_underlying_studies':9,'outcomes':dict(Counter(n['outcome']for n in notes)),'preparation_seconds':d['preparation_seconds'],'elapsed_seconds_including_coordination_and_receipt_writing':elapsed,'elapsed_minutes_per_new_scope':elapsed/60/9,'timing_limits':['One purposive ten-position R/U sample; not corpus-representative and not a capacity forecast.','Elapsed includes coordination and author note writing; not measured GPU/model active time.','Independent checking and all detailed extraction/integration remain outside this author timing.'],'coverage':{'freshly_hashed_document_copies':sum(len(c['files'])for c in d['inputs']),'distinct_source_hashes':len(d['files']),'full_pdf_text_pages_read':len(readset),'original_pdf_pages_viewed':len(viewset),'ooxml_documents_text_read':2,'full_papers_completed':0},'new_scientific_records':0,'new_approved_structure_recipe_pairs':0,'source_closures':0,'notes':['Seven synthesis-positive cases, including two from the insufficient/manual automated band.','Two theory-oriented scopes deferred with referenced-only or unresolved source status; no automatic corpus exclusion.','A formerly closed news scope reused without counting new completion.','No local atomic-coordinate dataset was established; this does not prove absence in all uninspected sources.'],'receipts':receipts}
