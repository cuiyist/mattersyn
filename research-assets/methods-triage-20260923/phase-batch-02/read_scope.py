...
ap=argparse.ArgumentParser();ap.add_argument('rank',type=int);ap.add_argument('--role',choices=['main','si']);ap.add_argument('--pages');ap.add_argument('--file-key');ap.add_argument('--render',action='store_true');a=ap.parse_args()
s=next(x for x in D['scopes'] if x['rank']==a.rank)
events=[]
for key in s['file_keys']:
 f=D['files'][key]
 if a.file_key and key!=a.file_key:continue
 ...
 if a.pages:
  ...
  for pno,t in pages.items():
   ...
   if lines:hits[pno]=[x[:180] for x in lines[:3]]
