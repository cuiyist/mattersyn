...
prior=json.loads((O.parent/'phase-batch-01/selection-manifest.json').read_text(encoding='utf-8'));prior_ids={x['group_id'] for x in prior['selected']}
selected=[];skips=[]
for s in ranked:
    if s['priority_band']!='C_phase_with_local_link_candidate':continue
    group=ledger['groups'].get(s['group_id'])
    reasons=[]
    if s['group_id'] in prior_ids:reasons.append('already_screened_phase_batch_01')
...
for s in selected:
    ...
    scopes.append(scope)
save(O/'selection-manifest.json',{'started_utc':'2026-09-23T13:17:14.085301+00:00','prepared_utc':datetime.now(timezone.utc).isoformat(),'priority_band_exact':'C_phase_with_local_link_candidate','selection':'next20 eligible in frozen inspection-rank order; no shared claims','selected':scopes,'skips':skips,'bound_inputs':{str(p):sha(p) for p in (RANK,DOC,LEDGER,BENCH)},'scientific_or_training_admission':False})
save(P/'source-inputs.json',{'files':files,'scopes':scopes})
save(O/'progress.json',{'phase':'source_preparation_complete','selected_scopes':20,'receipts_completed':0,'started_utc':'2026-09-23T13:17:14.085301+00:00'})
