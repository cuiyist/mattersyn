"""Proposed behavioral regression tests for MatterSyn.

This file is intentionally OUTSIDE the Site checkout and performs no file writes.
It imports the actual schema/helpers/validator and uses synthetic fixtures, plus
read-only checks of existing canonical records. Failing tests describe proposed
scientific/data-integrity contracts, not assertions that production was edited.

Run with the installed Python 3.13 matching Site dependencies:
  python -B research-assets/training-migration/test_dataset_proposed.py -v
Override MATTERSYN_SITE_ROOT if this proposal is moved to another workspace.
"""
import sys
...
SITE = Path(os.environ.get('MATTERSYN_SITE_ROOT', '[local path redacted]'))
