...
run([sys.executable,str(H/'build_sasongko_inventory.py')]);shutil.copy2(O/'inventory-summary.json',S/'data/inventory-summary.json')
...
for name in ['sasongko2025-protocol.mjs','protocol-visuals.mjs','crystal-viewer.mjs','source-evidence.mjs','illustrated-record.mjs']:run([node,'--check',str(S/'dist'/name)])
...
assert len(old)==656 and all(sha(S/'data/records'/n)==h for n,h in old.items())
assert len(exports)==6 and all(sha(S/'dist/data/exports'/n)==h for n,h in exports.items())
(O/'old-science-preservation.json').write_text(json.dumps({'status':'passed','unchanged_existing_records':656,'unchanged_training_exports':exports,'no_new_training_tasks':True},indent=2)+'\n',encoding='utf-8')
print('Established builders and scientific/static checks passed;656existing records andsixtraining exports are byte-preserved.')
