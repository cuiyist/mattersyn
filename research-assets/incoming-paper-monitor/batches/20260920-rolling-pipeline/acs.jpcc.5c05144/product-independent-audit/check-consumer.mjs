...
fs.writeFileSync(path.join(process.argv[3],proposalMode?'proposed-links-consumer-checks.json':'consumer-execution-checks.json'),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify({status:report.status,proposalMode,check_count:checks.length}));
