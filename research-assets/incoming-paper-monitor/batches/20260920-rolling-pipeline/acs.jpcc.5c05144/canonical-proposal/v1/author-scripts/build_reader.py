...
def prose(s):
 ...
 s=s.replace('25C synthesis','25 °C synthesis').replace('OAm70%','OAm 70%').replace('ODE>90%','ODE >90%')
...
for cat in['facts','materials','stocks','protocols','sample_contexts','figures','schemes','equations','conflicts','missingness','references']:
 for j,obj in enumerate(D[cat]):
  ...
  if cat=='facts':text=obj['claim']
  ...
  elif cat=='sample_contexts':
   note=('This is a quoted literature specimen, not a newly synthesized sample.'if obj['kind']in['literature context','literature_context']else'These quoted reference parameters do not provide a current QD refinement or atomic coordinates.'if obj['kind']=='literature_structure_context'else'This is an author interpretation, not an experimentally refined atomic model.'if obj['kind']=='author_model'else'The source does not establish an exact physical-aliquot or replicate association with other characterization contexts.')
   text=title+'. '+note
   ...
   text=obj.get('raw_citation',obj.get('citation',str(oid)))+' The cited full paper is not claimed to have been newly supplied or reviewed.';section='sources';title=('Main article'if oid.startswith('main-')else'Supporting information')+' reference '+oid.rsplit('-',1)[-1]
...
OPERATION_PROSE={
 'fa-charge':'Mix 0.1042 g formamidine acetate with 0.8 mL oleic acid and 3.2 mL 1-octadecene. These are whole-stock preparation charges; they are not the amount later injected into the QD reaction.',
 'fa-vacuum-hold':'Hold the FA precursor mixture at 60 °C for 30 min under vacuum. The numerical pressure, vessel, stirring speed and ramp are unreported.',
 'fa-nitrogen-hold':'Raise the FA precursor mixture to 135 °C and hold for 2 h under nitrogen. The final stock volume, concentration and solution coordination structure are unreported.',
 'lead-charge-degas':'Charge 0.075 mmol PbI2 in 2.5 mL 1-octadecene and degas at 60 °C for 30 min. The source describes the synthesis under nitrogen; this lead-precursor sentence does not explicitly specify vacuum. Numerical pressure is unreported.',
 'lead-preheat':'Raise the lead precursor mixture to 135 °C and hold for 30 min. The heating ramp is unreported.',
 'add-oa':'At 135 °C, inject the selected 0.4, 0.6 or 0.8 mL oleic acid charge. Select the value from the paired condition option; the three volumes are alternatives.',
 'add-oam':'After oleic acid, inject 0.2 mL oleylamine at 135 °C. Together with the selected OA charge, this gives the source OAm:OA volume ratio 1:2, 1:3 or 1:4.',
 'cool-equilibrate':'Cool to the selected 25, 50 or 100 °C reaction temperature and hold for 30 min. Preserve the ligand and washing conditions paired with that comparison; the source does not supply a complete Cartesian experiment matrix.',
 'inject-fa':'Inject 0.51 mL of the prepared FA precursor at the selected reaction temperature. This is an aliquot of the separately prepared stock. The injection rate is unreported.',
 'prompt-cool':'Promptly cool the post-injection mixture to room temperature. The cooling rate, numerical room temperature and post-injection growth dwell are unreported.',
 'add-wash':'Wash the crude dispersion with the selected 1:1, 1:10 or 1:20 acetonitrile:toluene formulation by volume. Select one formulation; absolute solvent volumes, premixing details and repetition count are unreported.',
 'first-spin':'Centrifuge at 12,000 rpm for 5 min and retain the precipitate. The supernatant is washing waste. Rotor radius, centrifugal force and centrifuge temperature are unreported.',
 'redisperse-hexane':'Redisperse the retained first precipitate in hexane. The solvent quantity and resulting concentration are unreported.',
 'second-spin':'Centrifuge the hexane redispersion at 6,000 rpm for 5 min. Retain the supernatant; the second precipitate is not the stated final product.',
 'store-supernatant':'Store the retained final supernatant for characterization. Storage temperature, duration, concentration and vessel are unreported.',
 'xrd-acquire':'Acquire powder diffraction with the X-pert3-Powder diffractometer (PANalytical). The comparison inputs are separate specimens. Radiation, scan range and step, mounting and broadening correction are unreported.',
 'trpl-acquire':'Measure time-resolved PL with a HORIBA Fluorolog-QM using 404 nm excitation and a 4 ns instrument response. Fit each specimen with a triple-exponential function and use the printed average-lifetime equation. Individual amplitudes, component lifetimes and fit errors are unreported.',
 'tem-acquire':'Acquire TEM images with a JEM-F200 (JEOL) and evaluate size distributions with ImageJ. The growth-temperature specimens remain separate. Voltage, grid, deposition details and particle count are unreported; no SAED or atomic-coordinate dataset is supplied.',
 'temperature-pl-acquire':'Measure PL from 80 to 430 K and fit Lorentzian spectra to obtain peak energy, width and intensity. The source names a Horiba LabRam and Linkam LNP96_S/LTS 420 temperature control. The 140 and 250 K phase boundaries are inferred outcomes, not extra acquisition setpoints. Atmosphere, ramp, equilibration, laser power and mounting are unreported.',
 'temperature-raman-acquire':'Acquire Raman spectra with a 633 nm pump over 50–300 cm−1. The attempted range is 80–200 K; the displayed range is 80–190 K because of PL/Raman overlap at higher temperature. The source names a Horiba LabRam and Linkam temperature control. Measurement atmosphere, laser power and thermal timing are unreported.',
 'thermal-aging-acquire':'Record PL at 350 K after 0, 30, 60, 90, 120, 150, 180 and 210 min, with a separately labeled 300 K reference spectrum. The normalized intensity fit belongs to the resulting observations; it does not define an acquisition setting or an absolute quantum yield. Detailed illumination and storage history are unreported.',
}
for pr in D['protocols']:
 rid=PRE+OWN['protocol_owner'][pr['id']]
 for op in pr['operations']:
  scope=next(x for x in CV['operations']if x['source_operation_id']==op['id']);text=OPERATION_PROSE[op['id']]
