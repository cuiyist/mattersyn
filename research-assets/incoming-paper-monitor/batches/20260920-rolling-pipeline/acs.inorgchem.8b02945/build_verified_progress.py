...
release=read(MON/'latest-publication.json');assert release['dataset_version']=='0.32.0'and release['status']=='published_verified'
active=read(D/'active-cutoff.json');out=D/'friedfeld-published-scope-20260920.json'
if not out.exists():
...
part=read(out);assert part['counts']['included_terminal_scopes']==31
...
for item in ed['current_work']:
 if item['short_label']=='Sasongko et al. (2025)':
  item['summary']='All 20 supplied main-paper and SI pages reviewed. The 19 records, 21 operations, nine explicitly paired condition options, 273 reader items and 17 selected source crops passed separate source and data audits. Reference crystal parameters remain separate from measured quantum-dot coordinates.'
  item['stages'][2]['detail']='The structured records and complete reader passed review. Molecular and stock illustrations and stage-specific apparatus diagrams are being prepared in parallel; each still requires its own audit.'
  item['stages'][1]['detail']='Preserved revision 2 passed; scientific quantities and original figure crops are unchanged.'
