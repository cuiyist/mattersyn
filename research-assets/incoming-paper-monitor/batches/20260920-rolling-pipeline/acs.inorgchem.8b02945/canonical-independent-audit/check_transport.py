...
def ck(k,v,detail=None):
 ...
 checks.append(row)
freeze=read(C/'package-freeze.json');m=read(C/'record-manifest.json');rcov=read(C/'reader/source-item-coverage.json');cov=read(C/'source-to-field-coverage.json');r=read(C/'reader/friedfeld2019.json')
source={'source-facts.json':read(S/'source-facts.json'),'source-inventory.json':read(S/'source-inventory.json'),'source-tables.json':read(F/'source-tables.json'),'original-assets-manifest.json':read(S/'original-assets-manifest.json'),'page-coverage.json':read(S/'page-coverage.json')}
...
ck('canonical exact frozen package',sha(C/'package-freeze.json')=='ff8767ef6a51f4184825de335dfe9906fab80e53b97e641fa534f6344f25a112')
...
for row in cov['source_units']:
 ...
 for b in row['canonical_bindings']:
  ...
  if unit['id'] in ['friedfeld2019-unit-graphical-abstract','friedfeld2019-unit-si-contents-1','friedfeld2019-unit-si-contents-2']:
   ck(unit['id']+' safe provenance subset',all(obj.get(k)==unit[k] for k in ['id','kind','title','summary','source_payload_ids']) and 'no full text' in obj.get('source_role',''))
...
ck('417 reader items',len(items)==len(itemmap)==417)
...
ck('independent all typed-field universe covered exactly',set(actual)==expected and len(actual)==len(expected)==2131,{'expected':len(expected),'actual':len(actual),'missing':list(expected-set(actual)),'extra':list(set(actual)-expected)})
ck('all 750 measurements linked',measurements=={(rid,x['id']) for rid,rec in records.items() for x in rec['measurements']} and len(measurements)==750)
...
assets=read(S/'original-assets-manifest.json');assetrows=assets.get('assets',assets.get('original_assets',[]));assetbyid={x['id']:x for x in assetrows};used=set();asset_keys=[]
for it in items:
...
ck('all 51 crops reachable',used==set(assetbyid) and len(used)==51,{'used':len(used),'original':len(assetbyid),'keys':asset_keys})
