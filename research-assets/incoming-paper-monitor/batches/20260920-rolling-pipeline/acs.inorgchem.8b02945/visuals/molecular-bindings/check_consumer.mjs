...
const O=path.dirname(fileURLToPath(import.meta.url)),A=path.join(O,'consumer-fixture'),P=path.join(O,'../..');
...
const records=new Map(read(path.join(P,'canonical-proposal/draft-v2/record-manifest.json')).records.map(x=>[x.record_id,read(x.path)]));
...
ck(slots===77,'All 77 slots executed');ck(api.chemicalEntry(data,'other-paper','oa')===undefined,'Cross-source exclusion');
...
ck(stockTransitions===16&&contextTransitions===16,'All 32 component transitions executed');
ck(dialogs===39&&threeD===21,'All 39 identities including 21 reference-geometry entries');
