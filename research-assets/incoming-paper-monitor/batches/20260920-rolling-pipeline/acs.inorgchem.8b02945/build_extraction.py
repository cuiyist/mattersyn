...
for j,(page,ctx,pos,a,x0,dx,amp,s)in enumerate(N.GAUSSIAN_BOXES):
 vals=[(a,'source area units unspecified','Area a'),(x0,'deg 2theta','X Position x0'),(dx,'deg 2theta','HWHM dx'),(amp,'source intensity units unspecified','ampl'),(s,'deg 2theta','s')]
...
for num,page,title,ctx,ann,box in N.SI_FIGURES:
 i=f'figure-s{num}';caption,box=caption_map[num];figures.append({'id':i,'role':'si','page':page,'title':title,'sample_context_ids':[ctx],'annotations':ann,'raw_caption':caption,'evidence':[ev('si',page,f'Actual printed FigureS{num} caption and graphic')],'raw_curves_digitized':False});crop_specs.append((i,'si',page,box,title))
