...
ap=argparse.ArgumentParser();ap.add_argument('--output-version',default='draft-v2');args=ap.parse_args();assert re.fullmatch('draft-v[0-9]+',args.output_version)
DRAFT=C/args.output_version;O=DRAFT/'reader';O.mkdir(exist_ok=True)
assert not(DRAFT/'package-freeze.json').exists()and not(DRAFT/'author-checkpoint-source-v1.json').exists(),'Preserve frozen versions and earlier checkpoints.'
...
CM=read(DRAFT/'record-manifest.json')
def effective(n):
    x=CM['effective_source_files'][n];assert sha(x['path'])==x['sha256'];return Path(x['path'])
D=read(effective('source-facts.json'));I=read(effective('source-inventory.json'));T=read(effective('source-tables.json'));A=read(effective('original-assets-manifest.json'));PC=read(effective('page-coverage.json'))
SOURCE_PASSED=CM['current_source_audit_status']=='passed'
CV=read(DRAFT/'source-to-field-coverage.json');OWN=read(DRAFT/'source-owner-map.json')
...
if SOURCE_PASSED:ITEM['overview-sources']['text']=ITEM['overview-sources']['text'].replace('Its independent source audit is still pending at this draft stage.','The effective extraction passed its distinct independent source audit. Canonical/reader review remains pending.')
...
for role in['main','si']:
    ...
    docs.append({'role':role,'filename':'10.1021_acs.inorgchem.8b02945'+('_si_1'if role=='si'else'')+'.pdf',
        ...
        'pages':[{'page':p['pdf_page'],'printed_page':802+p['pdf_page']if role=='main'else p['pdf_page'],'text_read':p['text_read'],'visual_review':p['visual_review'],'review_basis':'Frozen extraction and distinct passed source audit; canonical/reader audit is separate.'if SOURCE_PASSED else'Frozen extraction author page coverage; distinct source audit pending.','sections':[p['actual_reading_scope']]}for p in pp]})
...
if SOURCE_PASSED:
    rv['independent_audit']='Effective source extraction passed distinct scientific audit; canonical/reader review remains pending.'
    rv['supporting_information']['status']='matched_local_source_audit_passed_canonical_review_pending'
save(SID+'.json',rv)
