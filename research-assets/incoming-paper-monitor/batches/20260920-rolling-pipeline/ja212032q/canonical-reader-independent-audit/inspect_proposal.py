...
if mode=='keys':
 ...
 reader=load(P/'ghosh2012.json');print('reader',list(reader));print('sections',[(s['id'],len(s['items'])) for s in reader['reader_sections']]);print('exampleitem',json.dumps(reader['reader_sections'][0]['items'][0],ensure_ascii=False)[:6500])
elif mode=='reader':
 items=[i for s in load(P/'ghosh2012.json')['reader_sections'] for i in s['items']]
 ...
 r=load(C/('ghosh-2012-'+sys.argv[2]+'.json'))
 ...
 r=load(P/'ghosh2012.json');fmap={f['id']:f for s in r['reader_sections'] for i in s['items'] for f in i['facts']}
 ...
 for n in ['source-context','ligand-spectra','anneal-series']:
  x=load(C/('ghosh-2012-'+n+'.json'));print(n,[(m['id'],m['sample_id'],str(m['value']['value'])[:400]) for m in x['measurements'][:8]])
 ...
 print('OA',[(m['id'],m['sample_id']) for m in load(C/'ghosh-2012-ligand-spectra.json')['measurements'] if 'inset' in m['id']])
