...
I=P.parent/'intake-20260920T082035Z/intake-manifest.json'
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
intake=json.loads(I.read_bytes())
paper=next(p for p in intake['papers'] if p['paper_id']=='10.1021_ja212032q')
files=[]
...
for role in ['main','si']:
    ...
    for n,page in enumerate(reader.pages,1):
        ...
        pages.append({'pdf_page':n,'printed_page':None if role=='main' else 'S'+str(n),'text':text,'text_path':str(tp),'text_sha256':sha(tp),'render_path':str(out),'render_sha256':sha(out),'render_pixels':dims,'text_read':False,'visually_inspected':False})
    pdf.close()
