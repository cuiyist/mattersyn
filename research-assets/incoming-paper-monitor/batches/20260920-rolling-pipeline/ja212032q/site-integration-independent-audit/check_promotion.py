...
same(sha(P/'package-freeze.json'),'a5290e674534c727c012e34e331f27095dfc73ad4c397b30974072ae674a2712','exact promotion freeze')
...
removed=['All supplied main/SI extraction passed distinct source audit of revision 2. Canonical/reader review remains pending; this is an unapproved private draft. Historical pending flags in the frozen source payload describe its author freeze, before the separate audit.']
...
same(len(records),21,'21 records');same(Counter(r['reader_role'] for r in records.values()),Counter({'synthesis_route':5,'supporting_procedure':10,'contextual_observation':6}),'roles 5/10/6')
same(sum(len(r['operations']) for r in records.values()),33,'33 operations');same(sum(len(r['measurements']) for r in records.values()),585,'585 measurements')
old=bind(G/'public-review-proposal/v2/ghosh2012.json');reader=bind(P/'reader/ghosh2012.json');rr=copy.deepcopy(reader)
...
old_gap='The source extraction audit passed; independent canonical and reader approval is pending. Molecular, product, apparatus, browser and publication gates remain separate.'
...
same(reader['remaining_gaps'],[x for x in old['remaining_gaps'] if x!=old_gap]+[new_gap],'only historical reader gate replaced')
...
effective=bind(V/'metadata-correction-v3/effective-file-map.json')
def effective_read(name):
 row=effective[name];d=bind(row['path']);same(sha(row['path']),row['sha256'],'effective v3 input '+name);return d
...
same(len(reg['entries']),25,'25 molecular entries')
...
same(set(bm['sourceRecordSha256']),set(records),'source hash binding keys only Ghosh')
...
same(b,orig,'ALL 88 assignments quantities snapshots and scoped overrides unchanged')
...
same(ss,orig,'ALL stock source quantities component identities unchanged');same(len(sol['contexts']),3,'three stocks');same(sum(len(x['components']) for x in sol['contexts']),8,'eight components')
assets=pm['public_assets'];same(len(assets),85,'85 public files');same(Counter(x['kind'] for x in assets),Counter({'selected_original_scientific_crop':36,'reviewed_chemical_reference':48,'independently_audited_apparatus':1}),'36 crops48 chemicals1module')
crops={Path(x['path']).name:x for x in bind(G/'original-assets-manifest.json')['assets']};public=bind(V/'metadata-correction-v3/effective-public-assets.json');apparatus=bind(G/'visuals/apparatus/package-freeze.json');paths=set()
for row in assets:
 ...
 if row['kind']=='selected_original_scientific_crop':same(crops[p.name]['whole_source_page'],False,'not whole page '+rel);same(sha(p),crops[p.name]['sha256'],'audited crop '+rel)
 elif row['kind']=='reviewed_chemical_reference':same(sha(p),public[rel.removeprefix('assets/chemical-registry/')]['sha256'],'effective chemical v3 bytes '+rel)
...
result={'schema':'mattersyn-independent-promotion-delta-audit/1','author':'/root','auditor':'/root/peng1998_reader_assets','created_at':datetime.now(timezone.utc).isoformat(),'source_id':'ghosh2012','status':'passed' if not findings else 'open_findings','proposal_freeze_sha256':sha(P/'package-freeze.json'),'check_count':len(checks),'open_findings':findings,'counts':{'records':21,'routes_variants':5,'procedures':10,'observations':6,'operations':33,'measurements':585,'reader_links':links,'entries':25,'material_slots':88,'stocks':3,'stock_components':8,'public_files':85},'record_deltas':deltas,'reader_deltas':reader_deltas,'schema_results':schema,'reader_validator_errors':errors,'display_map_counts':{k:sum(map(len,v.values())) for k,v in maps.items()},'manual_scope':['Read complete root promotion script and every permitted status/gap transformation; genuine gaps remain and browser/publication are false.','Scientific arrays and reader content compared deeply, not merely quantities; only permitted metadata removed for equality.','Effective molecular v3 overlay used; all identities, 88 slot assignments, stock quantities and source captions unchanged.','Exact 85-file allowlist contains only independently passed selected crops, chemical assets and apparatus module; no full papers, full text or page images.','Display maps derive from this source reader only; separate product-context and actual integration gates remain pending.'],'bound_files':dict(sorted(bound.items())),'full_source_reaudit':False,'site_changed':False,'integration_approved':False,'browser_approved':False,'publication_approved':False,'training_approved':False}
save(A/'promotion-delta-audit.json',result);save(A/'promotion-delta-checks.json',{'checks':checks})
(A/'promotion-delta-audit.md').write_text(f"# Ghosh promotion delta audit\n\nStatus: **{result['status']}**; {len(checks)} independent checks, {len(findings)} open findings.\n\nThe exact frozen root proposal `{result['proposal_freeze_sha256']}` preserves all scientific content in 21 records, 33 operations and 585 measurement/context entries. Five route/variant, ten procedure and six observation roles are retained. Requested tasks and structure assets remain empty.\n\nAll reader scientific prose, source conflicts, tables, figures, typed fields and specimen associations remain unchanged. Only supported review metadata and stale approval-gate text differ. Effective molecular v3 references, all 88 material slots and three stock contexts/eight components are transported without scientific changes. The 85 public files are exactly 36 selected crops, 48 chemical assets and one separately audited apparatus module. Current canonical and reader validators run on private inputs.\n\nThis is a promotion/projection audit, not another full source audit. Product mapping, integrated code/browser, deployment and training approval remain separate.\n",encoding='utf8')
