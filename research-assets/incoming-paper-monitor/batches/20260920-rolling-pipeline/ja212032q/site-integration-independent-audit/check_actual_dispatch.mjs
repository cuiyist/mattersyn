...
const actual=await import(pathToFileURL(path.join(S,'dist/ghosh2012-protocol.mjs')));
const original=await import(pathToFileURL(path.join(G,'visuals/apparatus/ghosh2012-protocol.mjs')));
...
for(const r of records){
 if(r.lineage.source_group!=='ghosh2012'){
  for(const o of r.operations){check(actual.buildGhosh2012Scene(o,r)===null,'foreign record excluded '+r.record_id+' '+o.id);foreign++;}continue;
 }
 ...
 for(let i=0;i<r.operations.length;i++){
  const o=r.operations[i],scene=actual.buildGhosh2012Scene(o,r),expected=original.buildGhosh2012Scene(o,r);
  ...
  check(actual.buildGhosh2012Scene({...o,id:'unknown'},r)===null,'unknown op excluded '+o.id);
  check(actual.buildGhosh2012Scene(o,{...r,record_id:'wrong'})===null,'wrong record excluded '+o.id);
  stages++;rows+=scene.rows.length;parameters+=scene.rows.filter(x=>x.kind==='operation_parameter').length;alternatives+=scene.rows.filter(x=>x.kind==='alternative_schedule_parameter').length;
 }
