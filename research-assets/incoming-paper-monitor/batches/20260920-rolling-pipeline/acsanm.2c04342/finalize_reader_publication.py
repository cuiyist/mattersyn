...
plan=read(O/'release-endpoints.json');assert plan['dataset_version']=='0.30.0' and plan['source_id']=='matuhina2023'
...
assert v['expected_citation_count']==38
...
assert len(v['anonymous']['additional_withheld_paths'])==14 and {x['path'] for x in v['anonymous']['additional_withheld_paths']}==withheld
checks=v['anonymous']['checks'];assert len(checks)==plan['count']==219 and {c['path'] for c in checks}==set(plan['paths'])
...
assert all(x['http_status']==200 and x['bytes_match'] and x['doi_links']==38 for x in v['anonymous']['readmes'])
manifest=S/'dist/data/dataset-manifest.json';assert read(manifest)['dataset_version']=='0.30.0'
assert next(c['sha256'] for c in checks if c['path']=='data/dataset-manifest.json')==sha(manifest)
p=S/'data/paper-reviews/matuhina2023.json';assert sha(p)==r['baseline_reader_sha256'];before=read(p);after=copy.deepcopy(before)
assert before['paper_id']=='matuhina2023' and before['presentation_gates']['browser_render'] is True
