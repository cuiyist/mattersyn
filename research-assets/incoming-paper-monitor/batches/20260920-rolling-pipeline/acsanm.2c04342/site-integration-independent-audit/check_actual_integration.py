...
A=Path(__file__).resolve().parent;N=A.parent;O=N/'site-integration-proposal';P=O/'v2';C=N/'visuals/product-context';S=Path('[local path redacted]')
...
same(len(records),607,'607 records');same(set(records),{Path(x).stem for x in base['previous_records']}|set(new),'Exact complete record set')
...
manifest=bind(S/'dist/data/dataset-manifest.json');same(manifest['dataset_version'],'0.30.0','Dataset version');same(manifest['record_count'],607,'Manifest records');same(manifest['group_count'],38,'Manifest source groups')
...
for p in sorted((S/'dist/data/exports').glob('*.jsonl')):
 bind(p);rows=[json.loads(s)for s in p.read_text(encoding='utf8').splitlines()if s.strip()];task=p.stem;ids={r['record_id']for r in rows};expected={rid for rid,r in records.items()if eligibility(r)[task]['eligible']};same(ids,expected,'Training membership '+task);same(ids&set(new),set(),'No Matuhina training '+task);task_counts[task]=len(rows)
 baseline_ids={rid for rid,x in base['records'].items() if x['eligibility'][task]['eligible']};same(ids,baseline_ids,'Training membership unchanged from586-record baseline '+task)
...
reader=bind(S/'data/paper-reviews/matuhina2023.json');expected=read(P/'reader/matuhina2023.json')
...
expected=copy.deepcopy(reader);expected['review_scope_label']=source_review_scope(reader)['label'];same(bind(S/'dist/data/paper-reviews/matuhina2023.json'),expected,'Generated reader exact with derived coverage label');same(reader['review_scope'],'supplied_main_and_matched_si','Matched SI scope retained')
...
expected['entries'].extend(entries);same(bind(R/'registry.json'),expected,'Registry exact append 28 references and 7 symbols; all prior identities unchanged')
...
expected=copy.deepcopy(bind(B/'solution-components.json'));expected['contexts'].extend(read(P/'molecules/solution-components-additions.json')['contexts']);same(bind(R/'solution-components.json'),expected,'Exact five-stock append')
...
same(bind(R/'product-contexts.json'),expected,'Exact 47 product contexts; prior sample dispatch unchanged')
...
same(bind(S/'data/measurement-display.json'),expected,'Only Matuhina measurement classifications added');same(bind(S/'dist/data/measurement-display.json'),expected,'Public measurement map exact')
...
same({p.name for p in (S/'dist/assets/figures/matuhina2023').iterdir()if p.is_file()},{Path(x['public_path']).name for x in assets if x['kind']=='selected_original_scientific_crop'},'Exact 30 selected crops only')
urls=set()
def walk(x):
...
for rel in sorted(codefiles):
 ...
 if rel in delta['cache_revision_files']:expected=expected.replace('0.29.0-r1','0.30.0-r1').replace('0.29.0-r2','0.30.0-r2')
 ...
 else:ck('0.29.0-r' not in actual,'Generated HTML current cache epoch '+rel)
...
for p in [N/'import_reviewed_matuhina.py',N/'build_and_check_site.py',N/'build_matuhina_inventory.py',Path(__file__)]:bind(p)
findings=[x for x in checks if not x['passed']]
result={'schema':'mattersyn-independent-integration-transport-audit/1','source_id':'matuhina2023','author':'/root','auditor':'/root/norberg2004_extract','created_at':datetime.now(timezone.utc).isoformat(),'status':'passed'if not findings else'open_findings','passed':not findings,'open_findings':findings,'proposal_freeze_sha256':sha(P/'package-freeze.json'),'product_context_freeze_sha256':sha(C/'package-freeze.json'),'check_count':len(checks)+runtime['check_count'],'transport_check_count':len(checks),'runtime_check_count':runtime['check_count'],'counts':{'records':607,'old_records_byte_identical':586,'new_records_exact':21,'operations':39,'condition_rows':168,'operation_quantities':67,'option_quantities':30,'material_slots':55,'public_assets':82,'product_context_mappings':47,'training_tasks_added':0},'code_scope':code_evidence,'manual_scope':['Reviewed root import/build/inventory code and permitted source-specific dispatch/display changes.','Compared exact old and new canonical records, generated records, reader science, measurements, metadata, all public assets and append-only registry/sample maps.','Executed actual installed protocol and chemical-viewer functions in a minimal DOM harness; not a mounted-browser or appearance audit.','All six training export memberships and all586 prior eligibility results equal the independently saved pre-import baseline; no source or model eligibility promoted. No pre-import byte snapshot of exports was captured in this contribution.','The separately root-audited symbolic product package is checked for exact public projection transport; no new coordinate/model approval.'],'bound_files':dict(sorted(bound.items())),'site_changed_by_auditor':False,'browser_approved':False,'publication_approved':False,'training_approved':False}
