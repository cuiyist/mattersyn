...
const actual=await import(pathToFileURL(path.join(S,'dist/matuhina2023-protocol.mjs')));
const original=await import(pathToFileURL(path.join(G,'visuals/apparatus/matuhina2023-protocol.mjs')));
...
for(const r of records){
 if(r.lineage.source_group!=='matuhina2023'){
  for(const o of r.operations){check(actual.buildMatuhina2023Scene(o,r)===null,'foreign record excluded '+r.record_id+' '+o.id);foreign++;}continue;
 }
 ...
 for(let i=0;i<r.operations.length;i++){
  const o=r.operations[i],scene=actual.buildMatuhina2023Scene(o,r),expected=original.buildMatuhina2023Scene(o,r);
  ...
  check(actual.buildMatuhina2023Scene({...o,id:'unknown'},r)===null,'unknown op excluded '+o.id);
  check(actual.buildMatuhina2023Scene(o,{...r,record_id:'wrong'})===null,'wrong record excluded '+o.id);
...
check(stages===39,'39 actual stages');check(rows===168,'168 exact rows');check(parameters===67,'67 parameters');check(slots===55,'55 actual material bindings');check(alternatives===30,'30 grouped paired-condition quantities');
const bound={};for(const rel of ['dist/matuhina2023-protocol.mjs','dist/protocol-visuals.mjs','dist/chemical-viewer.mjs','dist/quantity-value.mjs','dist/assets/chemical-registry/registry.json','dist/assets/chemical-registry/bindings.json'])bound[path.join(S,rel)]=sha(path.join(S,rel));bound[fileURLToPath(import.meta.url)]=sha(fileURLToPath(import.meta.url));
