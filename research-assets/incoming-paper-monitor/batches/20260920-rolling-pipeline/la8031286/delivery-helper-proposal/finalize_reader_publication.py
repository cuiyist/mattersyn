...
plan=read(O/'release-endpoints.json');assert plan['dataset_version']=='0.31.0' and plan['source_id']=='pati2009'
...
assert v['expected_citation_count']==39
...
assert len(v['anonymous']['readmes'])==2 and {x['repository'] for x in v['anonymous']['readmes']}=={'mattersyn','mattersyn-site'}
withheld={f'assets/figures/{s}/pages/{d}-01.png' for s in ['heo2003','evans2010','morrison2017','lian2021','ghosh2012','sommer2020','matuhina2023','pati2009'] for d in ['main','si']}
assert len(v['anonymous']['additional_withheld_paths'])==16 and {x['path'] for x in v['anonymous']['additional_withheld_paths']}==withheld
checks=v['anonymous']['checks'];assert len(checks)==plan['count']==324 and {c['path'] for c in checks}==set(plan['paths'])
...
assert all(x['http_status']==200 and x['bytes_match'] and x['doi_links']==39 for x in v['anonymous']['readmes'])
manifest=S/'dist/data/dataset-manifest.json';assert read(manifest)['dataset_version']=='0.31.0'
assert next(c['sha256'] for c in checks if c['path']=='data/dataset-manifest.json')==sha(manifest)
p=S/'data/paper-reviews/pati2009.json';assert sha(p)==r['baseline_reader_sha256'];before=read(p);after=copy.deepcopy(before)
assert before['paper_id']=='pati2009' and before['presentation_gates']['browser_render'] is True
