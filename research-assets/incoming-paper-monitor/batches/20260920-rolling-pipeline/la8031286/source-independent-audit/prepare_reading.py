...
A=Path(__file__).resolve().parent;B=A.parent.parent;I=B/'intake-20260920T114139Z/intake-manifest.json'
sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
item=next(p for p in json.loads(I.read_text())['papers'] if p['paper_id']=='10.1021_la8031286')
copies=[]
...
for role in ['main','si']:
 ...
 for i,page in enumerate(d):
  t=A/'audit-text'/f'{role}-{i+1:02}.txt';t.write_text(page.get_text(sort=False),encoding='utf-8')
