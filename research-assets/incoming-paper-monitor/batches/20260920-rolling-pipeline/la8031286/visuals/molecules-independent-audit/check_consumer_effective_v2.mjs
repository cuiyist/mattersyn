...
const O=path.dirname(fileURLToPath(import.meta.url)),A=path.join(O,'../molecules'),P=path.join(A,'../..');
...
const overlay=path.join(A,'annotation-correction-v2');
...
const sha=p=>crypto.createHash('sha256').update(fs.readFileSync(resolvedPath(p))).digest('hex');
...
const records=new Map(read(path.join(P,'canonical-proposal/v2/record-manifest.json')).records.map(x=>[x.record_id,read(x.path)]));
...
const snapshotPath=path.join(A,'reference-snapshots/chemical-viewer.mjs');
const modulePath='[local path redacted]';
ck(sha(modulePath)===sha(snapshotPath),'Actual current viewer exactly equals frozen consumer');
...
ck(slots===45,'All 45 slots executed');ck(api.chemicalEntry(data,'other-paper','oa')===undefined,'Cross-source exclusion');
ck(api.chemicalEntry(data,[...records.keys()][0],'unmapped')===undefined,'Unmapped slot exclusion');
for(const raw of entries.values()){
 const e={...raw,sourceBindingCaption:'Independent audit source context for '+raw.id};
...
ck(dialogs===20&&threeD===6,'All 20 identities including 6 reference-geometry entries');
ck(JSON.stringify({registry,bindings})===before,'Original registry and private binding objects unchanged');
const bound_files=Object.fromEntries([modulePath,path.join(A,'registry-additions.json'),path.join(A,'bindings-proposal.json'),path.join(A,'solution-components-proposal.json'),path.join(A,'stock-component-map.json'),fileURLToPath(import.meta.url)].map(p=>[path.resolve(resolvedPath(p)),sha(p)]));
const result={schema:'mattersyn-independent-molecule-consumer-checks/1',independent_approval:false,auditor:'/root/norberg2004_extract',scaffolding:'Adapted author minimal-DOM fixture; independently executed against current Site consumer with added keyboard, close, post-3D symbolic fallback, exact-current-consumer and v2-record checks.',status:'passed',check_count:checks.length,counts:{slots,dialogs,threeD_entry_views:threeD,stockTransitions,contextTransitions},method:'Actual immutable chemical-viewer module executed with minimal DOM and 3Dmol data sink; approval flags changed only in in-memory copies. No mounted browser or rasterized WebGL claim.',bound_files,checks};
fs.writeFileSync(path.join(O,'consumer-checks-v2.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({status:result.status,checks:checks.length,counts:result.counts}));
