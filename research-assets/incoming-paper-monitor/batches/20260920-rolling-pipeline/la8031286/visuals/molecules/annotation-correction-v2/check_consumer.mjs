...
const O=path.dirname(fileURLToPath(import.meta.url)),A=path.dirname(O),P=path.join(A,'../..');
const effective=JSON.parse(fs.readFileSync(path.join(O,'effective-file-map.json'),'utf8'));
...
const bound_files=Object.fromEntries([modulePath,...Object.values(effective).map(x=>x.path),fileURLToPath(import.meta.url),path.join(O,'effective-file-map.json')].map(p=>[path.resolve(p),sha(p)]));
