"""Root-owned metadata projection from independently passed Pati packages."""
from pathlib import Path
from datetime import datetime,timezone
import copy,hashlib,json,re,shutil,sys
L=Path(__file__).resolve().parent;O=L/'site-integration-proposal/v1';S=L.parents[4]/'recipe-atlas'
...
assert not O.exists(),'Preserve proposals; create a revision instead.'
audits={}
for rel in ['source-independent-audit/independent-audit-v1.json','canonical-reader-independent-audit/independent-audit-v2.json','visuals/molecules-independent-audit/independent-audit-v2.json','visuals/apparatus-independent-audit/independent-audit.json','product-context-independent-audit/independent-audit.json']:
 p=L/rel;a=read(p);assert a['status']=='passed' and not a.get('open_findings');audits[rel]=sha(p)
scope='Complete supplied four-page main and four-page matched SI read and independently audited. Canonical/reader, molecular, apparatus and symbolic product-context audits passed. Three alcohol-specific precipitation routes, eleven supporting procedures and five observation records remain separate. As-prepared whole-powder identity is unresolved despite local CeO2 microscopy; calcination, DLS and XPS exposure histories retain their own scopes. Source conflicts are preserved. No atomic model, exact structure-recipe pair or new training task is admitted.'
stale='The complete supplied four-page main and four-page matched SI extraction passed distinct source audit. Canonical and reader proposals remain unapproved pending their separate review. Historical pending flags in retained source payloads describe their author-freeze stage.'
records={};deltas=[]
for row in read(L/'canonical-proposal/v2/record-manifest.json')['records']:
 ...
 for s in r['sources']:s.update(main_status='All four supplied main pages read, visually inspected and independently audited; source and canonical checks passed.',si_status='Matched four-page XPS SI fully read, visually inspected and independently audited. Pairing uses the main-paper declaration, matching content and embedded document metadata; SI has no title/byline.',reuse_status='Source-linked facts and selected scientific crops; original article, SI files, complete text and page scans remain local.')
 ...
 dest=O/'records'/src.name;save(dest,r);records[rid]=r;deltas.append({'record_id':rid,'original_path':str(src),'original_sha256':sha(src),'promoted_sha256':sha(dest),'reader_role':r['reader_role'],'scientific_values_unchanged':True})
assert len(records)==19 and sum(len(r['operations']) for r in records.values())==35
reader=read(L/'public-review-proposal/v2/pati2009.json');assert reader['review_scope']=='supplied_main_and_matched_si'
reader.update(coverage_status='supplied_main_and_si_review_complete',independent_audit=scope,source_review_promoted=True,publication_status='Source-reviewed contribution staged for website integration. Browser and deployment gates remain separate.',review_state='source_reviewed',training_note='No new training task is admitted. No atomic coordinates are supplied, as-prepared whole composition remains unresolved and analytical surface fits are not bulk formulas or exact sample-recipe structure labels.')
reader['audit_details']={'scope':scope,'audit_sha256':audits,'source_conflicts_resolved':False,'training_promotions':0}
reader['remaining_gaps']=[x for x in reader['remaining_gaps'] if x!='Complete supplied main/SI source extraction passed distinct audit; canonical/reader, visual, browser and publication gates remain separate.']
...
for item in reader['recipe_inventory']:item['status']='source_reviewed';item['gaps']=[x for x in item.get('gaps',[]) if x!=stale]
...
assert not re.search(r'[A-Z]:[\\/]|file://',json.dumps(reader));save(O/'reader/pati2009.json',reader)
public=[]
def asset(src,rel,kind):
 dest=O/'dist'/rel;dest.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(src,dest);public.append({'public_path':rel,'source_path':str(src),'sha256':sha(src),'kind':kind})
assets=read(L/'original-assets-manifest.json')['assets'];assert len(assets)==20
for a in assets:
 src=Path(a['path']);assert sha(src)==a['sha256'] and a['contains_complete_source_page'] is False and a['scientific_image_pixels_modified'] is False
 asset(src,'assets/figures/pati2009/'+src.name,'selected_original_scientific_crop')
urls=set()
def walk(x):
...
V=L/'visuals/molecules';effective=read(V/'annotation-correction-v2/effective-file-map.json')
def er(name):
 row=effective[name];p=Path(row['path']);assert sha(p)==row['sha256'];return read(p)
registry=er('registry-additions.json');bindings=er('bindings-proposal.json')
...
for rid,notes in bindings['bindingNotes'].items():
 ...
 for note in notes.values():note.update(binding_approved=True,independent_scientific_audit='Passed separate molecular/source-binding audit, including preserved nitrate annotation correction; scientific fields unchanged.')
...
solutions=er('solution-components-proposal.json');solutions['binding_approved']=True
for c in solutions['contexts']:c['binding_approved']=True
save(O/'molecules/solution-components-additions.json',solutions)
for rel,a in read(V/'annotation-correction-v2/effective-public-assets.json').items():
 src=Path(a['path']);assert sha(src)==a['sha256'];asset(src,'assets/chemical-registry/'+rel,'reviewed_chemical_reference')
asset(L/'visuals/apparatus/pati2009-protocol.mjs','pati2009-protocol.mjs','independently_audited_apparatus')
Q=L/'visuals/product-context';products=read(Q/'public-product-contexts-proposal.json');preg=read(Q/'registry-additions.json')
...
# Validate against the actual Site reader consumer with isolated proposed assets/records.
sys.path.insert(0,str(S/'scripts'));import build_paper_reviews as consumer
fixture=O/'consumer-fixture';(fixture/'data/records').mkdir(parents=True)
for p in (O/'records').glob('*.json'):shutil.copy2(p,fixture/'data/records'/p.name)
shutil.copytree(O/'dist/assets/figures',fixture/'dist/assets/figures')
...
counts={'records':19,'routes_and_variants':3,'supporting_procedures':11,'observations':5,'operations':35,'measurements':sum(len(r['measurements']) for r in records.values()),'selected_source_crops':20,'molecular_entries':20,'material_slots':45,'stock_components':12,'product_contexts':39,'product_context_symbols':11,'new_training_tasks':0,'exact_product_structure_pairs':0}
save(O/'promotion-manifest.json',{'schema':'mattersyn.pati_promotion_proposal/1','at':datetime.now(timezone.utc).isoformat(),'author':'/root','status':'frozen_metadata_overlay_pending_independent_delta_audit','source_audits':audits,'records':deltas,'counts':counts,'public_assets':public,'scope':scope,'removed_stale_quality_text':[stale],'effective_molecular_map_sha256':sha(V/'annotation-correction-v2/effective-file-map.json'),'display_map_counts':{k:sum(map(len,v.values())) for k,v in maps.items()},'pending_gates':['independent promotion delta audit','shared Site integration checks','actual integrated browser review','anonymous deployment verification']})
