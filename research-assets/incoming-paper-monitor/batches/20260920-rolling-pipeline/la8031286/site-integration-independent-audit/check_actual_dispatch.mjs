...
const actual=await import(pathToFileURL(path.join(S,'dist/pati2009-protocol.mjs')));
const original=await import(pathToFileURL(path.join(G,'visuals/apparatus/pati2009-protocol.mjs')));
...
for(const r of records){
 if(r.lineage.source_group!=='pati2009'){
  for(const o of r.operations){check(actual.buildPati2009Scene(o,r)===null,'foreign record excluded '+r.record_id+' '+o.id);foreign++;}continue;
 }
 ...
 for(let i=0;i<r.operations.length;i++){
  const o=r.operations[i],scene=actual.buildPati2009Scene(o,r),expected=original.buildPati2009Scene(o,r);
  ...
  check(actual.buildPati2009Scene({...o,id:'unknown'},r)===null,'unknown op excluded '+o.id);
  check(actual.buildPati2009Scene(o,{...r,record_id:'wrong'})===null,'wrong record excluded '+o.id);
...
check(stages===35,'35 actual stages');check(rows===167,'167 exact rows');check(parameters===43,'43 parameters');check(slots===45,'45 actual material bindings');check(alternatives===0,'No source alternative-schedule invention');
const bound={};for(const rel of ['dist/pati2009-protocol.mjs','dist/protocol-visuals.mjs','dist/chemical-viewer.mjs','dist/quantity-value.mjs','dist/assets/chemical-registry/registry.json','dist/assets/chemical-registry/bindings.json'])bound[path.join(S,rel)]=sha(path.join(S,rel));bound[fileURLToPath(import.meta.url)]=sha(fileURLToPath(import.meta.url));
