...
O=B/'canonical-proposal/v1';O.mkdir(parents=True,exist_ok=True)
...
def resolve(x,p):
 ...
 return x
I=read(B/'source-inventory.json');D=read(B/'source-facts.json');FREEZE=read(B/'source-extraction-freeze.json')
...
save('record-manifest.json',{'schema':'mattersyn-private-canonical-proposal/1','source_id':SID,'version':1,'created_at':datetime.now(timezone.utc).isoformat(),'author':'/root/norberg2004_extract','status':'author_draft_pending_source_and_canonical_audits','source_freeze_sha256':sha(B/'source-extraction-freeze.json'),'records':recs,'counts':counts,'coverage_sha256':sha(O/'source-to-field-coverage.json'),'validation_sha256':sha(O/'author-validation.json'),'author_script_sha256':sha(__file__)})
