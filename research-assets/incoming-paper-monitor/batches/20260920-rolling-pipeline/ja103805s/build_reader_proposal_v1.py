...
B=Path(__file__).resolve().parent;C=B/'canonical-proposal/v1';O=B/'public-review-proposal/v1';O.mkdir(parents=True,exist_ok=True)
...
I=read(B/'source-inventory.json');F=read(B/'source-facts.json');A=read(B/'selected-original-assets.json');PC=read(B/'page-coverage.json');MAN=read(C/'record-manifest.json');COV=read(C/'source-to-field-coverage.json');FR=read(B/'source-extraction-freeze.json')
...
def recsec(rid):
 ...
 if k in ['reagents','top-impurities','tbp-impurities','topse-impurities'] or k in P and P[k]['category'] in ['precursor_preparation','purification','stock_preparation']:return 'precursors'
 if k=='mechanism':return 'intuition'
 if k=='source-context':return 'sources'
 return 'protocol' if k in P else 'properties'
...
for rid,r in R.items():
 ...
 for n,m in enumerate(r['measurements']):
  ...
  attach(ii,rid,ptr,label,m['value'],m['sample_id']);MEAS[rid+'::'+m['id']]=ii['id']
...
for a in A['assets']:
 ...
 group='figures' if a['id'].startswith('figure-') else 'tables' if a['id'].startswith('table-') else 'schemes' if a['id'].startswith('scheme-') else 'equations'
...
inputs={str(p):sha(p) for p in [B/'source-extraction-freeze.json',B/'source-inventory.json',B/'source-facts.json',B/'page-coverage.json',B/'selected-original-assets.json',C/'record-manifest.json',C/'source-to-field-coverage.json',SITE/'scripts/build_paper_reviews.py',SITE/'dist/source-evidence.mjs']}
save('reader-manifest.json',{'schema':'mattersyn-private-reader-proposal/1','source_id':SID,'version':1,'author':'/root/norberg2004_extract','created_at':datetime.now(timezone.utc).isoformat(),'status':'private_author_draft_pending_independent_audit','input_hashes':inputs,'outputs':{n:sha(O/n) for n in ['evans2010.json','reader-bindings-proposal.json','source-item-coverage.json','author-validation.json']},'author_script_sha256':sha(__file__),'counts':counts,'published':False})
