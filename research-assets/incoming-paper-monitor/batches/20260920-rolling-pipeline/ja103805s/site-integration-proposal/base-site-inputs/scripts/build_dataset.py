...
def render_record(r,meta):
    ...
    if r['stocks']:
        ...
        for stock in r['stocks']:
            body+='<article class="stock-record" data-stock-id="'+esc(stock['id'])+'"><h4>'+esc(stock['name'])+'</h4><p>'+esc(stock['scope'])+'</p>'
    ...
    links=[dict(l,url=('../'+l['url'] if l['url'].startswith('paper-review.html') else record_href(l['url']))) for l in r['context_links']]
