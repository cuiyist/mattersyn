...
def q(raw,unit,meaning,role,page,locator,scale=1,conflicts=None):
    ...
    for word in ['approximately ','approx. ','ca. ','about ','within ','after ']:
        if t.startswith(word):
            t=t[len(word):];approx=word in ['approximately ','approx. ','ca. ','about '];comparison={'within ':'<='}.get(word);break
    if meaning.startswith('approx '):approx=True
