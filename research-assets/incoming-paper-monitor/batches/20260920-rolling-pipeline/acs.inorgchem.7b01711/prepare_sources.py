...
I=P.parent/'intake-20260920T055502Z/intake-manifest.json'
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
intake=json.loads(I.read_bytes())
paper=next(p for p in intake['papers'] if p['paper_id']=='10.1021_acs.inorgchem.7b01711')
files=[]
...
docs=[]
for role in ['main','si']:
    f=next(f for f in files if f['role_candidate']==role and f['source_collection']=='incoming')
    ...
    for n,page in enumerate(reader.pages,1):
        ...
        pages.append({'pdf_page':n,'printed_page':12919+n if role=='main' else 'S'+str(n),'text':text,'text_path':str(tp),'text_sha256':sha(tp),'render_path':str(out),'render_sha256':sha(out),'render_pixels':dims,'text_read':False,'visually_inspected':False})
    pdf.close()
