...
for u in I['inventory_units']:
    o=pointer(D,u['json_pointer']);check(o['id']==u.get('source_object_id',u['id']),'inventory pointer '+u['id'])
...
docs={r:pypdfium2.PdfDocument(str(p)) for r,p in source_paths.items()};cache={};pixel_count=0
for asset in M['assets']:
    ...
    if key not in cache:
        pg=docs[key[0]][key[1]-1];bitmap=pg.render(scale=key[2]);cache[key]=bitmap.to_pil().convert('RGB').copy();bitmap.close();pg.close()
