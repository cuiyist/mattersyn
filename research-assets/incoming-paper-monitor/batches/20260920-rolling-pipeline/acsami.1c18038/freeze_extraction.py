...
ck('PS reported unit remains unknown',f['chemicals']['quantities'][-1]['unit']is None)
...
ck('frozen inventory pointers resolve',all(s[u['json_pointer'].split('/')[1]][int(u['json_pointer'].split('/')[2])]['id']==u['source_object_id']for u in inv['inventory_units']))
ck('inventory unit ids globally unique',len({u['id']for u in inv['inventory_units']})==len(inv['inventory_units']))
ck('inventory unit keys preserve scientific identity',all(u['id']==u['type']+':'+u['source_object_id']for u in inv['inventory_units']))
ck('N2 applies specifically to TGA',next(o for p in s['protocols']for o in p['operations']if o['id']=='bulk-xps-tga')['action']=='Acquire XPS; acquire TGA under N2')
...
notes+='\nRevision 2 preserves the initial freeze in source-extraction-revision-1. The only source-object change clarifies that nitrogen applies to TGA, not XPS. Inventory unit IDs now carry a type prefix and a separate unchanged source_object_id; scientific object IDs and JSON pointers are unchanged. The source-correction-history file records these bounded deltas.\n'
(P/'extraction-notes.md').write_text(notes,encoding='utf-8')
names=['prepare_sources.py','source_author_data.py','build_extraction.py','freeze_extraction.py','prepare_source_revision_2.py','source-correction-history.json','complete-source-payloads.json','source-facts.json','source-inventory.json','source-tables.json','page-coverage.json','pairing-review.json','relevance-screening.json','original-assets-manifest.json','source-consistency-diagnostics.json','author-visual-review.json','author-validation.json','extraction-notes.md']
files=[P/n for n in names]+list((P/'private').rglob('*'))+list((P/'source-render').glob('*.png'))+list((P/'reader-assets').glob('*.png'))+list((P/'source-extraction-revision-1').glob('*'))
