...
A=Path(__file__).resolve().parent;N=A.parent;O=N/'site-integration-proposal';P=O/'v1';C=N/'product-context-proposal';S=Path('[local path redacted]')
...
def frozen(p):
 ...
 if 'files' in f:
  ...
  for n,h in f['bound_files'].items():bind(n);same(sha(n),h,'Frozen bound input '+n)
 return f
base=bind(A/'pre-integration-baseline.json');imp=bind(O/'site-import-manifest.json');delta=bind(O/'code-delta.json')
...
same(len(records),586,'586 records');same(set(records),{Path(x).stem for x in base['previous_records']}|set(new),'Exact complete record set')
...
manifest=bind(S/'dist/data/dataset-manifest.json');same(manifest['dataset_version'],'0.29.0','Dataset version');same(manifest['record_count'],586,'Manifest records');same(manifest['group_count'],37,'Manifest source groups')
...
task_counts={}
for rel,x in base['training_export_snapshots'].items():
 p=S/rel;bind(p);same(sha(p),x['sha256'],'Training export byte-identical '+rel);rows=[json.loads(s)for s in p.read_text(encoding='utf8').splitlines()if s.strip()];task=p.stem;ids={r['record_id']for r in rows};expected={rid for rid,r in records.items()if eligibility(r)[task]['eligible']};same(ids,expected,'Training membership '+task);same(ids&set(new),set(),'No Sommer training '+task);task_counts[task]=len(rows)
...
reader=bind(S/'data/paper-reviews/sommer2020.json');expected=read(P/'reader/sommer2020.json')
expected['presentation_gates'].update(site_integration=True,symbolic_product_contexts=True)
expected['remaining_gaps']=[x for x in expected['remaining_gaps']if x!='Source, canonical/reader, molecular and apparatus audits passed. Symbolic product-context binding, integrated browser and publication gates remain separate.']
...
expected=copy.deepcopy(reader);expected['review_scope_label']=source_review_scope(reader)['label'];same(bind(S/'dist/data/paper-reviews/sommer2020.json'),expected,'Generated reader exact with derived coverage label');same(reader['review_scope'],'supplied_main_only_si_unverified','SI remains unverified')
R=S/'dist/assets/chemical-registry';B=A/'baseline/dist/assets/chemical-registry'
expected=bind(B/'registry.json');expected=copy.deepcopy(expected);entries=read(P/'molecules/registry-additions.json')['entries']+read(C/'registry-additions.json')['entries']
for e in entries:e.update(published=True,binding_approved=True)
expected['entries'].extend(entries);same(bind(R/'registry.json'),expected,'Registry exact append 28 references and 3 symbols; all prior identities unchanged')
...
expected=copy.deepcopy(bind(B/'solution-components.json'));expected['contexts'].extend(read(P/'molecules/solution-components-additions.json')['contexts']);same(bind(R/'solution-components.json'),expected,'Exact seven-stock append')
expected=copy.deepcopy(bind(B/'product-contexts.json'));extra=read(C/'product-contexts-additions.json')
...
same(bind(R/'product-contexts.json'),expected,'Exact 77 phase-component mappings; prior sample dispatch unchanged')
expected=copy.deepcopy(bind(A/'baseline/data/measurement-display.json'))
...
same(bind(S/'data/measurement-display.json'),expected,'Only Sommer measurement classifications added');same(bind(S/'dist/data/measurement-display.json'),expected,'Public measurement map exact')
...
for e in read(C/'registry-additions.json')['entries']:bind(R/e['svgPath']);same(sha(R/e['svgPath']),e['assetHashes']['svgPath'],'Installed phase symbol '+e['id'])
same({p.name for p in (S/'dist/assets/figures/sommer2020').iterdir()if p.is_file()},{Path(x['public_path']).name for x in assets if x['kind']=='selected_original_scientific_crop'},'Exact 20 selected crops only')
urls=set()
def walk(x):
...
for rel in sorted(codefiles):
 independent=A/'baseline'/rel;rootbase=O/'base-site-inputs'/rel;p=independent if independent.exists() else rootbase;old=bind(p).read_text(encoding='utf8');expected=old
 ...
 if rel in delta['cache_revision_files']:expected=expected.replace('0.28.0-r1','0.29.0-r1').replace('0.28.0-r2','0.29.0-r2')
 ...
 else:ck('0.28.0-r' not in actual,'Generated HTML current cache epoch '+rel)
 code_evidence.append({'file':rel,'baseline':'independent' if independent.exists() else 'root preserved','installed_sha256':sha(S/rel)})
merged={'dist/assets/chemical-registry/'+x for x in ['registry.json','bindings.json','solution-components.json','product-contexts.json']}|{'data/measurement-display.json','dist/data/dataset-manifest.json','dist/data/validation-report.json'}
...
allowed={'lian2021':'Chemical/atomic visual binding is separately pending.','ghosh2012':'Source-qualified identity only; molecular/product visual binding is separately pending.','sommer2020':'Chemical identity and source role only; independently qualified molecular/component bindings remain pending.'}
...
for rid,r in new.items():
 for product in r['products']:
  for note in product['notes']:
   ...
   if note.startswith('Source context object: '):ck('<details' in rendered and 'Source context object: {' not in rendered,'Expandable source provenance '+rid+'/'+product['sample_id'])
   else:same(rendered,'<p>'+html.escape(note)+'</p>','Other product notes exact '+rid+'/'+product['sample_id'])
inventory=bind(S/'dist/data/inventory-summary.json');same(inventory,bind(S/'data/inventory-summary.json'),'Public inventory exact');same(inventory,bind(O/'inventory-summary.json'),'Private inventory exact');same(inventory['summary']['canonical_records'],586,'Inventory record count');same(inventory['summary']['synthesis_route_variant_records'],114,'Inventory routes')
row=next(x for x in inventory['per_paper']if x['source_group']=='sommer2020');same(row['record_ids'],sorted(new),'Inventory source IDs');same(row['measurement_entry_count'],532,'Inventory source measurements');same(row['synthesis_route_variant_count'],3,'Inventory three routes');same(row['procedure_count'],9,'Inventory nine procedures');same(row['contextual_observation_count'],7,'Inventory seven observations');same(row['si_status'],'unverified','Inventory SI missingness')
index=bind(S/'dist/data/paper-review-index.json');ck('sommer2020' in json.dumps(index),'Source reader indexed')
...
for p in [N/'import_reviewed_sommer.py',N/'build_and_check_site.py',N/'build_sommer_inventory.py',Path(__file__)]:bind(p)
findings=[x for x in checks if not x['passed']]
result={'schema':'mattersyn-independent-integration-transport-audit/1','source_id':'sommer2020','author':'/root','auditor':'/root/peng1998_reader_assets','created_at':datetime.now(timezone.utc).isoformat(),'status':'passed'if not findings else'open_findings','passed':not findings,'open_findings':findings,'proposal_freeze_sha256':sha(P/'package-freeze.json'),'product_context_freeze_sha256':sha(C/'package-freeze.json'),'check_count':len(checks)+runtime['check_count'],'transport_check_count':len(checks),'runtime_check_count':runtime['check_count'],'counts':{'records':586,'old_records_byte_identical':567,'new_records_exact':19,'operations':31,'condition_rows':168,'operation_quantities':50,'option_quantities':155,'material_slots':62,'public_assets':60,'phase_component_mappings':77,'training_tasks_added':0},'code_scope':code_evidence,'manual_scope':['Reviewed root import/build/inventory code and permitted source-specific dispatch/display changes.','Compared exact old and new canonical records, generated records, reader science, measurements, metadata, all public assets and append-only registry/sample maps.','Executed actual installed protocol and chemical-viewer functions in a minimal DOM harness; not a mounted-browser or appearance audit.','All six training exports are byte-identical to the independently saved pre-import baseline; no source or model eligibility promoted.','The separately root-audited symbolic phase package is checked for exact transport only; this does not re-audit my own authored phase science.'],'bound_files':dict(sorted(bound.items())),'site_changed_by_auditor':False,'browser_approved':False,'publication_approved':False,'training_approved':False}
...
(A/'integration-transport-audit.md').write_text(f"# Sommer installed transport audit\n\nStatus: **{result['status']}**. {len(checks)} transport and {runtime['check_count']} actual-module checks; {len(findings)} open findings.\n\nAll 567 earlier records and six training exports remain byte-identical. Nineteen new records, 532 measurements, 31 stages, 62 material slots, seven stocks, 77 phase-component mappings and 60 public files preserve the passed proposals. Scientific fields, source conflicts, unknown whole compositions and unverified SI remain unchanged.\n\nActual installed protocol/chemical dispatch is exercised in a minimal DOM. Root owns mounted-browser visual checks and release verification. This audit does not approve atomic structures or training.\n",encoding='utf8')
