...
proposal=bind(O/'conditional-publication-rule-proposal.json');plan=bind(O/'release-endpoints.json');script=bind(N/'finalize_reader_publication.py');current=bind(S/'data/paper-reviews/sommer2020.json');browser=bind(A/'browser-gate-delta-audit.json')
ck('Distinct root proposal author',proposal['author']=='/root')
ck('Current baseline exactly approved browser reader',sha(S/'data/paper-reviews/sommer2020.json')==proposal['baseline_reader_sha256']==browser['after_reader_sha256'])
...
ck('94 unique endpoint plan',plan['dataset_version']=='0.29.0'and plan['source_id']=='sommer2020'and len(plan['paths'])==len(set(plan['paths']))==plan['count']==94)
...
ck('17 source endpoints',sum('sommer2020' in x or 'sommer-2020' in x for x in paths)==17)
expected=[{'pointer':'/presentation_gates/publication','before':False,'after':True},{'pointer':'/publication_status','before':current['publication_status'],'after':'Published after independent source, data, illustration and integrated browser reviews; exact deployed GitHub Pages commit and anonymous public bytes verified. Declared SI remains locally unlocated and unverified.'}]
...
ck('Original pre-hardening script preserved',sha(O/'finalizer-pre-proof-hardening.py')=='74e600ad44b3760a329678d2e00e73957682f455445f7e86cee030447a41dea8')
bind(O/'finalizer-pre-proof-hardening.py');bind(O/'conditional-publication-rule-pre-hardening.json')
...
withheld=[f'assets/figures/{s}/pages/{d}-01.png'for s in ['heo2003','evans2010','morrison2017','lian2021','ghosh2012','sommer2020']for d in ['main','si']]
proof={'status':'passed','site_commit':'a'*40,'build':{'commit':'a'*40,'status':'built'},'expected_citation_count':37,'repositories':[{'name':x,'public':True,'commit_matches':True,'anonymous_status':200}for x in ['mattersyn','mattersyn-site']],'anonymous':{'authenticated':False,'cookies_used':False,'checks':[{'path':x,'http_status':200,'matches_checked_local_bytes':True,'redirect_stays_on_site':True,'sha256':sha(S/'dist'/x)}for x in paths],'additional_withheld_paths':[{'path':x,'http_status':404}for x in withheld],'excluded_complete_page_http_status':404,'readmes':[{'repository':x,'http_status':200,'bytes_match':True,'doi_links':37}for x in ['mattersyn','mattersyn-site']]}}
...
after=writes[str(S/'data/paper-reviews/sommer2020.json')];reversed_=copy.deepcopy(after)
for x in expected:
