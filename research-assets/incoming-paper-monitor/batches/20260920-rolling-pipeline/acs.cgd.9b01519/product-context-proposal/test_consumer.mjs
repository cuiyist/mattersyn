...
for(const[rid,rows]of Object.entries(payload.recordContexts)){
 const host=node('host');await fn(host,{record_id:rid,lineage:{source_group:'sommer2020'}});
 ...
 for(let i=0;i<rows.length;i++){
  ...
  ck(rid+'/'+i+' exact component label',select.children[i].text===row.label+' · '+row.sample_id&&row.label.includes('phase component'));
