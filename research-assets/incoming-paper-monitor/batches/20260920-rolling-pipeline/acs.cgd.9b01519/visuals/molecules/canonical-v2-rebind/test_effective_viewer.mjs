...
for(const[rid,m]of Object.entries(bindings.recordBindings))for(const[mid,id]of Object.entries(m)){
 ...
 ck(`${rid}/${mid} private entry lookup`,chemicalEntry(data,rid,mid)===entry);
 ck(`${rid}/${mid} initially unapproved`,note.binding_approved===false);
 ...
 ck(`${rid}/${mid} whitelisted identity`,resolved.name===note.viewOverrides.name);
 ck(`${rid}/${mid} no model override`,resolved.model3dPath===entry.model3dPath&&resolved.svgPath===entry.svgPath);
 ck(`${rid}/${mid} entry unchanged`,chemicalEntry(data,rid,mid)===entry);
}
ck('Unknown source excluded',chemicalEntry(data,'not-sommer','zn-nitrate')===undefined);
ck('Unknown slot excluded',chemicalEntry(data,Object.keys(bindings.recordBindings)[0],'unmapped')===undefined);
const report={status:'passed_author_execution_checks',check_count:checks.length,checks,module_path:modulePath,module_sha256:crypto.createHash('sha256').update(fs.readFileSync(modulePath)).digest('hex'),mounted_browser_approval:false,independent_audit:false};
