"""Independent Sommer source/canonical/reader transport checks; no author file writes."""
...
def quantity_sample(f,x):
 if isinstance(x,dict):
  m=re.match(r'^([MAISD][0-9]+(?:/[MAISD][0-9]+)*)\s',x.get('meaning',''))
...
def qcheck(q,x,context):
 ...
 for k in ['conflict_ids','gap_ids']:
  for identity in x.get(k,[]):
   # C3 concerns duration and G3 absent operational quantities, neither changes the exact 220 C set point.
   if context=='sommer2020-acs-settings sommer-2020-acs-route/operations/1/parameters/set_point':continue
...
ck(sha(C/'package-manifest.json')=='336d7db442acd6433aa08453cb1b530d58eb46c133588b8deb7c2c3cb3e52ffd','expected_freeze','v1')
...
records={p.stem:read(p) for p in sorted(C.glob('sommer-2020-*.json'))};cov=read(C/'source-to-field-coverage.json');lossless=read(C/'lossless-source-map.json')
for key in src:
 ck(lossless[key]==src[key],'lossless_source',key)
ck(lossless['tables']==tables,'lossless_source','tables')
for t in tables:
 for ix,row in enumerate(t['rows'],1):
  identity=t['id']+'-r'+str(ix).zfill(2)+'-sample'
  cells[identity]=({'id':identity,'raw_text':row['row_label'],'value':row['row_label'],'evidence':row['row_label_evidence']},t,row)
...
for row in cov['table_cells']:
 ...
 for b in row['canonical_bindings']:
  ...
  m=ptr(records[b['record_id']],b['pointer'].rsplit('/',1)[0]);ck(m['sample_id']==slug(tr['sample_id']),'cell_sample',row['source_cell_id'])
...
for row in cov['source_objects']:
 ...
 if row['category']=='material':
  ...
  if identity in ['mw-low-base','mw-middle-base','mw-high-base']:ck('do not represent fresh nitrate/water charges' in actual['scope'],'flattened_stock_scope',identity)
...
for rid,r in records.items():
 for option in r['condition_options']:
  if option['id'].startswith('table-1-'):
   original=all_table_rows[option['id'][8:]]
   for key,q in option['parameters'].items():qcheck(q,next(c for c in original['cells'] if c['column']==key),rid+'/'+option['id']+'/'+key)
  elif option['id'] in ['mw-profile-1','mw-profile-2']:
   original=facts['sommer2020-'+option['id']]
   for key,q in option['parameters'].items():qcheck(q,next(x for x in original['quantities'] if slug(x['meaning']).replace('-','_')==key),rid+'/'+option['id']+'/'+key)
for row in cov['source_units']:
 ...
 for b in row['canonical_bindings']:
  ...
  if b['pointer'].startswith('/measurements/') and unit['kind'] not in ['fact','sample_context']:
   value=q.get('value')
   if isinstance(value,str) and value.startswith('{'):ck(json.loads(value)==original,'complete_object_payload',unit['id'])
   elif unit['kind']=='reference':ck(value==original['citation_as_extracted'],'complete_reference_payload',unit['id'])
   elif unit['kind']=='conflict':ck(value==original['description'],'complete_conflict_payload',unit['id'])
   elif unit['kind']=='gap':ck(value==original['description'],'complete_gap_payload',unit['id'])
...
for rid,r in records.items():
 ...
 ck(r['lineage']['source_group']=='sommer2020','single_source_group',rid)
...
reader=read(P/'sommer2020.json');rcov=read(P/'source-item-coverage.json');bindings=read(P/'reader-bindings-proposal.json')
...
for p,x in walk(reader):
 if isinstance(x,dict) and 'public_asset' in x:
  ...
  if name in assetmap:
   a=assetmap[name];ck(x.get('sha256',x.get('public_asset_sha256'))==a['sha256'],'public_asset_hash',p);ck(x['public_asset']=='assets/figures/sommer2020/'+name+'.png','public_asset_path',p)
