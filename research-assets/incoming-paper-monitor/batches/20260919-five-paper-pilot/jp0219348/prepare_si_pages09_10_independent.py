...
R=Path(__file__).resolve().parent;O=R/'reader-assets/si-pages09-10-independent';O.mkdir(exist_ok=True);sha=lambda p:hashlib.sha256(Path(p).read_bytes()).hexdigest()
...
(R/'si-pages09-10-independent-assets.json').write_text(json.dumps({'auditor':'/root/norberg2004_extract','source_path':s['path'],'source_sha256':s['sha256'],'assets':assets},indent=2)+'\n',encoding='utf-8')
