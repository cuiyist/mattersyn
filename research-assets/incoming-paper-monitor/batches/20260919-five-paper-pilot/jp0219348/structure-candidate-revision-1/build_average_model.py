...
for block in table3['blocks']:
    for row in block['rows']:
        ...
        tolerance=max(3*(uncertainty or 0),0.006 if q['unit']=='degree' else 0.0006)
        geometry.append({'label':label,'calculated':calculated,'source_value':reported,'source_raw':row['raw_cells'][1],
            'unit':q['unit'],'absolute_difference':abs(calculated-reported),'tolerance':tolerance,
            'within_source_uncertainty_and_printed_rounding':abs(calculated-reported)<=tolerance})
assert all(g['within_source_uncertainty_and_printed_rounding'] for g in geometry)
...
notes=[
    ...
    'Fd-3m origin choice2 is resolved by the printed Wyckoff labels and coordinates; all nine multiplicities and all23 Table3 geometries are independently testable.',
    ...
    'The framework is represented by colocated statistical Si/Al site components of0.5 each, following Table2 footnote c. This is the averaged Si96Al96 model, not a resolution of nominal Si100Al92.',
