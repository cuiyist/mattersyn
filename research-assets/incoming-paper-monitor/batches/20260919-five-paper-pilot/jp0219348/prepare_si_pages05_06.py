"""Extract unaltered embedded scan pixels for a new bounded SI numerical chunk."""
...
O=B/'reader-assets/si-numerical-pages05-06'
...
for page in [5,6]:
    ...
    render=Image.open(B/f'si-{page:02d}.png');rw,rh=render.size
...
out={'schema':'mattersyn-si-numerical-evidence/1','author':'/root','created_at':datetime.now(timezone.utc).isoformat(),'source_id':'heo2003','source_path':d['path'],'source_sha256':d['sha256'],'assets':assets}
(B/'si-pages05-06-assets.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf8')
print(json.dumps({'pages':[5,6],'assets':len(assets),'dimensions':assets[0]['pixels']}))
