"""Extract native source scan pixels for the separately scoped SI 9-10 chunk."""
...
B=Path(__file__).resolve().parent;O=B/'reader-assets/si-numerical-pages09-10'
...
preserve={str(p):sha(p) for p in B.iterdir() if p.is_file() and p.suffix in ['.json','.tsv','.md'] and not p.name.startswith('si-pages09-10')}
...
(B/'si-pages09-10-assets.json').write_text(json.dumps(out,indent=2)+'\n',encoding='utf8')
(B/'si-pages09-10-preserved-inputs.json').write_text(json.dumps(preserve,indent=2)+'\n',encoding='utf8')
print(json.dumps({'pages':[9,10],'assets':len(assets),'source_sha256':d['sha256'],'preserved_files':len(preserve)}))
