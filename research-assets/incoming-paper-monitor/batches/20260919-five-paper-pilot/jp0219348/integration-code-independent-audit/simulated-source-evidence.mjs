...
export function sourceItemCard(item,{compact=false,sourceDoi=null,sourceId=null}={}){
 ...
 if(sourceId==='heo2003'&&item.id==='inventory-supporting_tables-0'){
  const d=el('details'),area=el('div');d.append(el('summary','Browse all 1,209 supporting reflections'),area);let started=false;
  d.addEventListener('toggle',async()=>{if(!d.open||started)return;started=true;try{const {mountHeoReflections}=await import('./heo2003-reflection-viewer.mjs');await mountHeoReflections(area);}catch(e){area.textContent='Reflection viewer unavailable. Consult the downloadable source table.';console.error(e);}});card.append(d);
 }
 return card;
}
