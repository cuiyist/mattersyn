"""Package the root's manually read SI pages5–6; independent audit is separate."""
...
def check(label,ok):
    ...
    assert ok,label
check('Exact page/block scope',list(blocks)==['5L','5R','6L','6R'])
rows=[];tsv=[];neg=[];ids=[]
...
expected_negative={'5L':{2:'-6519.44',8:'-8964.25',25:'-771.95'},'5R':{4:'-249.67',9:'-289.23',36:'-6332.25',45:'-798.39'},'6L':{8:'-7029.53',12:'-723.66',14:'-5031.03',19:'-6810.55',23:'-4445.18',27:'-2211.99',45:'-55595.68'},'6R':{3:'-6338.71',9:'-291.05',14:'-1495.79',16:'-3376.37',19:'-2263.07',35:'-4441.21'}}
expected=[(f'si-p{int(k[:-1]):02d}-{k[-1]}-r{rn:03d}-Fobs2',s) for k,d in expected_negative.items() for rn,s in d.items()]
check('All20negative observations retained',[(n['cell_id'],n['raw_text']) for n in neg]==expected)
check('180rows/1260distinct cells',len(rows)==180 and len(set(ids))==1260)
previous=read(B/'si-reflections-transcription.json')['rows']+read(B/'si-pages03-04-transcription.json')['rows']
prior_hkl={tuple(r['hkl']) for r in previous};new_hkl={tuple(r['hkl']) for r in rows}
check('180distinct new hkl; no prior350overlap or merging',len(new_hkl)==180 and not new_hkl.intersection(prior_hkl))
...
write(out[1],{'schema':'mattersyn-si-reflection-transcription-chunk/1','source_id':'heo2003','title_as_printed':'Observed and Calculated Structure Factors Squared with Esds for In66-X','author':'/root','created_at':now,'pages':[5,6],
...
files=[B/(stem+'-author-blocks.json'),B/(stem+'-assets.json'),B/'prepare_si_pages05_06.py',Path(__file__),out[0],out[1]]
write(out[2],{'schema':'mattersyn-si-numerical-chunk-author-checkpoint/1','source_id':'heo2003','author':'/root','created_at':now,'status':'author_source_comparison_complete_independent_numerical_audit_pending',
    'actual_scope':{'pages_transcribed':[5,6],'native_crop_pages_visually_read':[5,6],'rows':180,'numeric_cells':1080,'marker_cells':180,'total_cells':1260,'negative_Fobs2':20,'rows_per_page_block':{k:len(v) for k,v in blocks.items()}},
    ...
    'complete_document_transcription':False,'remaining_untranscribed_pages':list(range(7,15)),'remaining_unaudited_pages':list(range(5,15)),'remaining_row_count':None,
...
print(json.dumps({'chunk':'pages05-06','rows':len(rows),'numeric_cells':1080,'markers':180,'negative_values':len(neg),'checks':len(checks),'checkpoint_sha256':sha(out[2]),'independent_audit':'pending'}))
