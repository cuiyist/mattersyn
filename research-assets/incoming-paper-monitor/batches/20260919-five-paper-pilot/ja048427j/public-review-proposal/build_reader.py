...
if AM is not ORIGINAL_AM:FROZEN[str(O/'reader-original-assets-manifest.json')]=sha(O/'reader-original-assets-manifest.json')
...
OP_PROSE=read(O/'operation-reader-prose.json')
assert set(OP_PROSE)=={op['id'].removeprefix(PRE) for r in R.values() for op in r['operations']}
...
for rid,r in R.items():
    for n,op in enumerate(r['operations']):
        ...
        add(sec,iid,op['label'],OP_PROSE[op['id'].removeprefix(PRE)],units,'method_context','source_operation',ce(op['evidence']))
...
def asset_item(a,key=None,group=None,label=None,sids=None,notes=None):
    key=key or a['id'];role=a['source_role'];page=a['pdf_page'];full=a['kind']=='full_page';public_name=Path(a.get('replaces_asset',{}).get('path',a['path'])).name;public='assets/figures/norberg2004/'+public_name
    if full:public='assets/figures/norberg2004/pages/'+public_name
    ...
    if a.get('reader_specific_rerender'):
        entry['asset_provenance']['replaces_archived_render']={'sha256':a['replaces_asset']['sha256'],'renderer':a['replaces_asset']['renderer'],'original_manifest_sha256':a['replaces_asset']['original_manifest_sha256'],'reason':'Reader-specific PDFium render preserves degree-C glyphs from unchanged original PDF; original crop boundaries and source hash are unchanged.'}
        entry['source_review_basis']='Original source content is covered by the passed full-source audit. The reader-specific PDFium renders received author contact-sheet and targeted native glyph inspection; independent reader render review remains pending.'
