...
for ii in flat:
    ...
    specimen_links=ii['sample_scope']['canonical_sample_links']
    check('no duplicate specimen pointers '+ii['id'],len(specimen_links)==len({(j['record_id'],j['json_pointer']) for j in specimen_links}))
...
for number in [4,5,7]:
    check('figure-specific context on evidence card '+str(number),{(j['record_id'],j['json_pointer']) for j in items[f'figure-{number}']['sample_scope']['canonical_sample_links']}=={(j['record_id'],j['json_pointer']) for j in asset_by[SID+f'-figure-{number}']['canonical_sample_links']})
check('pH inequality not invented','does not establish whether' in items['figure-6']['text'] and 'differs from' not in items['figure-6']['text'])
check('uninspected upstream reference wording','main article directs readers to references 7 and 25' in items['gap-2']['text'])
