...
for f in inv['figures']:
    add(FIG_SECTION[f['number']],'figure-'+str(f['number']),f'Figure {f["number"]}: '+f['title'],f['scope'],[{'pdf_page':f['pdf_page'],'section':f'Figure {f["number"]}'}], 'author_model_figure' if f['number'] in [2,3] else 'mixed_measurement_and_author_model' if f['number']==6 else 'original_experimental_figure')
...
for n,g in enumerate(inv['remaining_gaps'],1):add('sources',f'gap-{n}',GAP_TITLES[n-1],g,uby[f'remaining_gaps-{n}']['evidence'],'source_missingness','curator_interpretation')
...
for c in inv['evidence_conflicts']:add('sources','conflict-'+c['id'],CONFLICT_TITLES[c['id']],c['description'],c['evidence'],'unresolved_source_conflict','curator_interpretation')
...
for cid,keys in {
    ...
    for key in keys:
        ...
        items[key]['notes'].append(prose(c['description']))
