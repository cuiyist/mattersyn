...
checks=[
 ...
 'All36scenes inspected on6contact sheets;4changed examples reopened at full scene resolution.',
...
save(V/'author-visual-check.json',{'schema':'mattersyn.private-apparatus-author-check/1','source_id':'nagasaki2004','author':'/root','created_at':datetime.now(timezone.utc).isoformat(),'status':'passed_author_visual_source_mapping_pending_independent_audit',
    ...
    'counts':{'scenes':36,'contact_sheets':6,'changed_full_scene_views':4,'mechanical_selection_checks':len(m['checks']),'scientific_author_scopes':len(checks),'bound_files':len(bound)},'scientific_scopes':checks,'bound_files':bound,
