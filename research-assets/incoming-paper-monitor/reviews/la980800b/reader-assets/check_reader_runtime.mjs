...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/stiger1999.json'),'utf8'));
...
globalThis.location={search:'?id=stiger1999',hash:''};
...
check('All 144 source items executed',cards.length===144&&items.length===144);
...
check('Fifteen distinct original assets accessible',filenames.length===15);
check('All three numbered equations accessible',[1,2,3].every(n=>filenames.includes('equation-0'+n+'.png')));
check('Actual SAED detail and table and scheme accessible',['figure-06b-saed.png','table-01.png','scheme-01.png'].every(f=>filenames.includes(f)));
...
if(input?.events.input){input.value='Schottky';input.events.input();check('Source search filters meaningfully',cards.some(n=>!n.hidden)&&cards.some(n=>n.hidden));input.value='';input.events.input();check('Cleared search restores all items',cards.every(n=>!n.hidden));}else check('Source search event wired',false);
const digest=p=>crypto.createHash('sha256').update(fs.readFileSync(p)).digest('hex');
const failures=checks.filter(c=>!c.passed);
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled local DOM execution of actual reader modules; no browser geometry, apparatus acceptance or publication claim.',reader_items:cards.length,original_asset_links:imageLinks.length,unique_original_assets:filenames.length,original_asset_filenames:filenames,checks_passed:checks.length-failures.length,check_count:checks.length,failures,checks,artifact_sha256:Object.fromEntries(['paper-review.mjs','source-evidence.mjs','data/paper-reviews/stiger1999.json'].map(p=>['dist/'+p,digest(path.join(dist,p))]))};
