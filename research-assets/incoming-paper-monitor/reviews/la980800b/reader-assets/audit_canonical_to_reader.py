"""Bounded actual Stiger reader/export audit. Only writes this private directory.
...
SITE=B.parents[3]/'recipe-atlas'; SID='stiger1999'; P='stiger-1999-'
...
def main():
    ...
    ledger=read(SITE/'data/paper-reviews/stiger1999.json');generated=read(SITE/'dist/data/paper-reviews/stiger1999.json')
    proposal=read(B/'public-review-proposal/stiger1999.json');coverage=read(B/'public-review-proposal/source-item-coverage.json');source=read(B/'source-audit.json')
    ...
    routeids={P+'electrodeposition'}
    check('8 records, 145 measurements, 36 operations',len(records)==8 and sum(len(r['measurements']) for r in records.values())==145 and sum(len(r['operations']) for r in records.values())==36)
    check('One route, four procedures, two controls and one observation',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=1,procedure=4,protocol_variant=2,observation=1))
    check('Final independent audit covers all 8 records',science['status'].startswith('passed') and not science.get('open_findings') and not science.get('failed_checks') and set(audited)==set(records))
    for rid,r in records.items():
        ...
        for m in r['measurements']:
            ...
            elif rid.endswith('open-circuit-control'):labels=['open-circuit-control']
            ...
            if m['id'].startswith('char-'):
                ...
                expected_sample='shared-'+'-and-'.join(old['sample_id']) if isinstance(old['sample_id'],list) else old['sample_id']
    ...
    check('143 characterization rows plus two control observations',len(records[P+'characterization']['measurements'])==143 and len(joins)==145)
    check('All 143 source characterization facts represented in reader',set(rawchar)==set(factitems))
    check('144 reader items and 202 source-audit crosswalk units',len(items)==144 and len(coverage['source_audit_unit_map'])==202)
    for u in coverage['source_audit_unit_map']:check('Source unit '+u['source_unit_id'],ptr(source,u['source_audit_json_pointer'])['id']==u['source_unit_id'] and all(bool(ptr(ledger,p)) for p in u['reader_json_pointers']))
    ...
    for key,need in {'source-specific-electrolytes':['1 mM','10 mM','1.5 mM'],'transferred-particles':['10–25 Å','pulse duration is not stated'],'saed-indexing':['actual electron-diffraction','reference relative intensities'],'density-time-series':['cm⁻² ms⁻¹','error-bar meaning'],'summary-scope':['cm⁻² s⁻¹','conflicts'],'early-current-models':['undefined A','starred concentration','no visible F'],'histogram-1':['2.2 nm','2.4 nm','5 ms'],'conflict-14':['four panels','fifth']}.items():check(key+' critical semantic boundaries',all(v in items[key]['text'] for v in need))
    ...
    check('15 originals: 9 figures, 3 equations, one SAED detail, table and scheme',len(assets)==15 and [len(ledger[k]) for k in ['figures','equations','source_notes','tables','schemes']]==[9,3,1,1,1])
    for a in assets:
    ...
    check('Actual reader DOM execution passed',runtime['status']=='passed' and runtime['reader_items']==144 and runtime['unique_original_assets']==15)
    ...
    check('All 8 records in one evaluation group and split',len({groups[rid] for rid in records})==1 and len({mr[rid]['split'] for rid in records})==1)
    ...
    for formula in ['Ag/Si','Ag','Si']:
        ...
        check(formula+' exactly one active-paper route',actual_routes==routeids)
        ...
        check(formula+' correct direct route relationship',bool(routeids&set(hub['direct_record_ids']))==(formula=='Ag/Si'))
        check(formula+' periodic lookup element identity retained',set(formula.split('/'))<=set(meta.get('elements',hub.get('elements',[]))))
    ...
    for p in [SITE/'data/paper-reviews/stiger1999.json',SITE/'dist/data/paper-reviews/stiger1999.json',SITE/'dist/data/dataset-manifest.json',SITE/'dist/data/validation-report.json',SITE/'data/inventory-summary.json',SITE/'dist/data/materials-index.json',SITE/'dist/paper-review.mjs',SITE/'dist/source-evidence.mjs',SITE/'dist/material-guide.mjs',SITE/'dist/material-hub.mjs',SITE/'scripts/dataset_lib.py',SITE/'scripts/build_dataset.py',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/source-item-coverage.json',B/'public-review-proposal/stiger1999.json',OUT/'reader-runtime-check.json']:bind(p)
