...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/banerjee2003.json'),'utf8'));
...
globalThis.location={search:'?id=banerjee2003',hash:''};
...
check('All 143 source items executed',cards.length===143&&items.length===143);
...
check('Eight main and one SI original figure cards',figureCards.length===9);
...
check('All 15 original assets accessible',allAssets.length===15&&filenames.length===15&&allAssets.every(a=>filenames.includes(a.public_asset.split('/').pop())));
for(const a of allAssets)check('Original asset exists '+a.id,fs.existsSync(path.join(dist,a.public_asset)));
check('Nine main and one SI page displayed',document.getElementById('review-summary').textContent.includes('10 pages')&&ledger.documents.length===2&&ledger.documents.find(d=>d.role==='main')?.page_count===9&&ledger.documents.find(d=>d.role==='si')?.page_count===1);
...
if(input?.events.input){input.value='TDPA';input.events.input();check('Source search filters meaningful subset',cards.some(n=>!n.hidden)&&cards.some(n=>n.hidden));input.value='';input.events.input();check('Cleared search restores all items',cards.every(n=>!n.hidden));}else check('Source search wired',false);
...
const {buildBanerjee2003Scene}=await import(pathToFileURL(path.join(dist,'banerjee2003-protocol.mjs')).href+'?audit='+Date.now());
const {buildBanerjee2003Scene:privateScene}=await import(pathToFileURL(path.join(privateBase,'visuals/banerjee2003-protocol.mjs')).href+'?audit='+Date.now());
...
for(const rid of allRecordIds){
 ...
 for(const op of record.operations){
  operationCount++;const scene=buildBanerjee2003Scene(op,record),expected=privateScene(op,record);
...
check('All 39 operations have source-specific scenes',operationCount===39);
...
for(const rid of allRecordIds){
 ...
 for(const [index,op] of record.operations.entries()){
  ...
  const tree=descend(host),copy=tree.find(x=>x.className==='protocol-copy'),art=tree.find(x=>x.className==='protocol-art protocol-art-banerjee2003');
  check(rid+'/'+op.id+' mounted source-specific artwork',art?.innerHTML===buildBanerjee2003Scene(op,record).svg);
...
check('All 39 actual mounted stages exercised',mountedOperations===39);
check('All 53 actual condition quantities displayed',mountedQuantities===53);
const failures=checks.filter(c=>!c.passed);
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled DOM execution of actual source-reader and apparatus modules, exact original-asset accessibility and source-defined molecular bindings. No browser geometry or publication claim.',reader_items:cards.length,unique_original_assets:filenames.length,operation_count:operationCount,mounted_operations:mountedOperations,mounted_quantities:mountedQuantities,binding_count:bindingCount,checks_passed:checks.length-failures.length,check_count:checks.length,failures,console_errors:consoleErrors,checks,artifact_sha256:Object.fromEntries(['paper-review.mjs','source-evidence.mjs','protocol-visuals.mjs','quantity-value.mjs','banerjee2003-protocol.mjs','data/paper-reviews/banerjee2003.json','assets/chemical-registry/bindings.json','assets/chemical-registry/registry.json'].map(p=>['dist/'+p,digest(path.join(dist,p))]))};
