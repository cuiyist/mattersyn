...
import {buildBanerjee2003Scene} from './banerjee2003-protocol.mjs';
...
for(const file of fs.readdirSync(path.join(B,'canonical-drafts')).filter(n=>n.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){
  const before=JSON.stringify({o,r}),s=buildBanerjee2003Scene(o,r),key=r.record_id+'/'+o.id;
...
const out={module:'banerjee2003-protocol.mjs',module_sha256:sha(fs.readFileSync(path.join(V,'banerjee2003-protocol.mjs'))),record_count:14,operation_count:scenes.length,checks,scenes};
