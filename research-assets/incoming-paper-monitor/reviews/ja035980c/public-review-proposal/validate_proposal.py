...
r=read(O/'banerjee2003.json');drafts={p.stem:read(p) for p in (B/'canonical-drafts').glob('*.json')};manifest=read(B/'reader-assets/crop-manifest.json');source=read(B/'source-manifest.json');audit=read(B/'source-audit.json');checks=[]
...
ck('all reference entries',[x['reference_number'] for x in r['referenced_methods']],list(range(1,44)));ck('all figures',sorted(a['id'] for a in r['figures']),sorted(['figure-'+str(n) for n in range(1,9)]+['si-infrared']))
