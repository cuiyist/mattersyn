...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='braun2001'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/braun2001.json');reader=read(S/'data/paper-reviews/braun2001.json');generated=read(S/'dist/data/paper-reviews/braun2001.json')
 ...
 ck('9 records / 45 operations / 75 measurements',len(records)==9 and sum(len(r['operations']) for r in records.values())==45 and sum(len(r['measurements']) for r in records.values())==75)
 ck('3 routes / 3 procedures / 3 observations',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=3,procedure=3,observation=3))
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=braun2001' in raw)
 ...
 ck('85 reader items intact',set(items)==set(pi) and len(items)==85)
 ...
 ck('132 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==132)
 ...
 ck('8 original assets and category counts',set(assets)==set(crops) and len(assets)==8 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[4,0,0,4])
 ...
 for num in [2,3,4]:ck('Figure '+str(num)+' exact system scope',assets['figure-'+str(num)]['sample_links']==['braun-2001-system-i','braun-2001-system-ii','braun-2001-system-iii'])
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==85 and runtime['unique_original_assets']==8 and runtime['operation_count']==45 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/braun2001.json',S/'dist/data/paper-reviews/braun2001.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/braun2001-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/crystal-viewer.mjs',S/'dist/material-hub.mjs',S/'dist/assets/chemical-registry/product-bindings.json',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/braun2001.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 8 originals, material hubs, 45 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':9,'operations':45,'measurements':75,'reader_items':85,'source_units':132,'original_assets':8,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Braun 2001 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
