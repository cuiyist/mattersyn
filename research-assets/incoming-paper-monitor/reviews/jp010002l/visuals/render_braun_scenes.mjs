...
import {buildBraun2001Scene} from './braun2001-protocol.mjs';
...
for(const file of fs.readdirSync(path.join(B,'canonical-drafts')).filter(n=>n.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){
  const before=JSON.stringify({o,r}),s=buildBraun2001Scene(o,r),key=r.record_id+'/'+o.id;
...
const out={module:'braun2001-protocol.mjs',module_sha256:sha(fs.readFileSync(path.join(V,'braun2001-protocol.mjs'))),record_count:9,operation_count:scenes.length,checks,scenes};
