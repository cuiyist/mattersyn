...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='besson2002'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/besson2002.json');reader=read(S/'data/paper-reviews/besson2002.json');generated=read(S/'dist/data/paper-reviews/besson2002.json')
 ...
 ck('12 records / 36 operations / 98 measurements',len(records)==12 and sum(len(r['operations']) for r in records.values())==36 and sum(len(r['measurements']) for r in records.values())==98)
 ck('2 literature protocols / 1 protocol variant / 7 procedures / 2 observations',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=2,protocol_variant=1,procedure=7,observation=2))
 ...
 expected_structure={
  'mesoscopic_structure','film_thickness','model_derived_average_pore_diameter','xrd_mesoscopic_c_parameter','gap_size_derived_average_cds_diameter',
  'image_derived_mesoscopic_a_parameter','image_derived_mesoscopic_c_parameter','image_derived_mesoscopic_c_over_a','mesoscopic_space_group','local_atomic_structure_assignment',
  '0002_intensity_evolution','0002_intensity_relative_to_calcined_host','0002_peak_shift','0002_peak_width_evolution',
  'figure2_displayed_two_theta_range','figure2_intensity_display_multiplier','figure2_trace_labels',
  'figure3a_scale_bar_length','figure3b_scale_bar_length','figure3c_scale_bar_length','figure3d_index_labels','image_power_definition',
  'cadmium_depth_distribution','cds_volume_fraction','mesopore_volume_fraction','mesopore_volume_filling_fraction','structure_scale_distinction'}
 ...
 ck('Optical confinement threshold stays under Properties',not structural('author_weak_confinement_size_threshold'))
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=besson2002' in raw)
  ...
  for n,m in enumerate(r['measurements']):
   ...
   if m['property'] in expected_structure or m['property']=='author_weak_confinement_size_threshold':
 ...
 ck('130 reader items intact',set(items)==set(pi) and len(items)==130)
 ...
 ck('173 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==173)
 ...
 ck('10 original assets and category counts',set(assets)==set(crops) and len(assets)==10 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[4,0,0,6])
 ...
 for num,target in [(2,'xrd'),(3,'hrtem'),(4,'pl-silicon')]:ck('Figure '+str(num)+' exact specimen scope',assets['figure-'+str(num)]['sample_links']==['besson-2002-'+target])
 ck('Missing SI stays explicit',reader['supporting_information']['status']=='not_located_or_matched' and not any(d['role']=='si' for d in reader['documents']))
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==130 and runtime['unique_original_assets']==10 and runtime['operation_count']==36 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/besson2002.json',S/'dist/data/paper-reviews/besson2002.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'data/measurement-display.json',S/'dist/data/measurement-display.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/besson2002-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/crystal-viewer.mjs',S/'dist/material-hub.mjs',S/'dist/assets/chemical-registry/product-bindings.json',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/besson2002.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 10 originals, material hubs, 36 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':12,'operations':36,'measurements':98,'reader_items':130,'source_units':173,'original_assets':10,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Besson 2001 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
