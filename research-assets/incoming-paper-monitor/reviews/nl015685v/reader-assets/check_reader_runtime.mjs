...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/besson2002.json'),'utf8'));
...
globalThis.location={search:'?id=besson2002',hash:''};
...
check('All 130 source items executed',cards.length===130&&items.length===130);
...
check('All 10 original assets accessible',allAssets.length===10&&filenames.length===10&&allAssets.every(a=>filenames.includes(a.public_asset.split('/').pop())));
for(const a of allAssets)check('Original asset exists '+a.id,fs.existsSync(path.join(dist,a.public_asset)));
check('Six-page main-only review displayed',document.getElementById('review-summary').textContent.includes('6 pages')&&ledger.documents.length===1&&ledger.documents[0].role==='main');
check('SI remains unverified',ledger.supporting_information.status==='not_located_or_matched');
check('SI search limitation explicit',/No locally matched SI/.test(cardMap.get('coverage')?.textContent||''));
check('Mesostructure parameter distinction explicit',cardMap.get('mesostructure-comparison')?.textContent.includes('6.8 nm')&&cardMap.get('mesostructure-comparison')?.textContent.includes('7.2 nm'));
check('Figure 4 assignment conflict explicit',cardMap.get('pl-figure-conflict')?.textContent.includes('caption'));
check('Power spectrum distinguished from SAED',cardMap.get('image-power')?.textContent.includes('not a selected-area electron diffraction'));
const input=nodes.find(n=>n.tagName==='input'&&n.placeholder==='Chemical, condition, sample, technique or interpretation');
if(input?.events.input){input.value='copolymer';input.events.input();check('Source search filters meaningful subset',cards.some(n=>!n.hidden)&&cards.some(n=>n.hidden));input.value='';input.events.input();check('Cleared search restores all items',cards.every(n=>!n.hidden));}else check('Source search wired',false);
...
const {buildBesson2002Scene}=await import(pathToFileURL(path.join(dist,'besson2002-protocol.mjs')).href+'?audit='+Date.now());
const {buildBesson2002Scene:privateScene}=await import(pathToFileURL(path.join(privateBase,'visuals/besson2002-protocol.mjs')).href+'?audit='+Date.now());
...
for(const rid of allRecordIds){
 ...
 for(const op of record.operations){
  operationCount++;const scene=buildBesson2002Scene(op,record),expected=privateScene(op,record);
...
check('All 36 operations have source-specific scenes',operationCount===36);
...
for(const rid of allRecordIds){
 ...
 for(const [index,op] of record.operations.entries()){
  ...
  const tree=descend(host),copy=tree.find(x=>x.className==='protocol-copy'),art=tree.find(x=>x.className==='protocol-art protocol-art-besson2002');
  check(rid+'/'+op.id+' mounted source-specific artwork',art?.innerHTML===buildBesson2002Scene(op,record).svg);
...
check('All 36 actual mounted stages exercised',mountedOperations===36);
check('All 44 actual condition quantities displayed',mountedQuantities===44);
const failures=checks.filter(c=>!c.passed);
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled DOM execution of actual source-reader and apparatus modules, exact original-asset accessibility and source-defined molecular bindings. No browser geometry or publication claim.',reader_items:cards.length,unique_original_assets:filenames.length,operation_count:operationCount,mounted_operations:mountedOperations,mounted_quantities:mountedQuantities,binding_count:bindingCount,checks_passed:checks.length-failures.length,check_count:checks.length,failures,console_errors:consoleErrors,checks,artifact_sha256:Object.fromEntries(['paper-review.mjs','source-evidence.mjs','protocol-visuals.mjs','quantity-value.mjs','besson2002-protocol.mjs','data/paper-reviews/besson2002.json','assets/chemical-registry/bindings.json','assets/chemical-registry/registry.json'].map(p=>['dist/'+p,digest(path.join(dist,p))]))};
