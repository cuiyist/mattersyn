...
import {buildBesson2002Scene} from './besson2002-protocol.mjs';
...
for(const file of fs.readdirSync(path.join(B,'canonical-drafts')).filter(n=>n.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){
  const before=JSON.stringify({o,r}),s=buildBesson2002Scene(o,r),key=r.record_id+'/'+o.id;
...
const out={module:'besson2002-protocol.mjs',module_sha256:sha(fs.readFileSync(path.join(V,'besson2002-protocol.mjs'))),record_count:12,operation_count:scenes.length,checks,scenes};
