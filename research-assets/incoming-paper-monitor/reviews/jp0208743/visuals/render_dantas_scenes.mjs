...
import {buildDantas2002Scene} from './dantas2002-protocol.mjs';
...
for(const file of fs.readdirSync(path.join(B,'canonical-drafts')).filter(n=>n.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){
  const before=JSON.stringify({o,r}),s=buildDantas2002Scene(o,r),key=r.record_id+'/'+o.id;
...
const out={module:'dantas2002-protocol.mjs',module_sha256:sha(fs.readFileSync(path.join(V,'dantas2002-protocol.mjs'))),record_count:16,operation_count:scenes.length,checks,scenes};
