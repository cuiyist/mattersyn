...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/dantas2002.json'),'utf8'));
...
globalThis.location={search:'?id=dantas2002',hash:''};
...
check('All 101 source items executed',cards.length===101&&items.length===101);
...
check('All 11 original assets accessible',allAssets.length===11&&filenames.length===11&&allAssets.every(a=>filenames.includes(a.public_asset.split('/').pop())));
for(const a of allAssets)check('Original asset exists '+a.id,fs.existsSync(path.join(dist,a.public_asset)));
check('Five-page main-only review displayed',document.getElementById('review-summary').textContent.includes('5 pages')&&ledger.documents.length===1&&ledger.documents[0].role==='main');
...
check('Size versus radius ambiguity explicit',cardMap.get('size-radius-boundary')?.textContent.includes('radius'));
check('Crucible source anomaly explicit',cardMap.get('crucible')?.textContent.includes('aluminum'));
check('No invented Raman acquisition',cardMap.get('raman-boundary')?.textContent.includes('Raman-shift'));
const input=nodes.find(n=>n.tagName==='input'&&n.placeholder==='Chemical, condition, sample, technique or interpretation');
if(input?.events.input){input.value='AFM1';input.events.input();check('Source search filters meaningful subset',cards.some(n=>!n.hidden)&&cards.some(n=>n.hidden));input.value='';input.events.input();check('Cleared search restores all items',cards.every(n=>!n.hidden));}else check('Source search wired',false);
...
const {buildDantas2002Scene}=await import(pathToFileURL(path.join(dist,'dantas2002-protocol.mjs')).href+'?audit='+Date.now());
const {buildDantas2002Scene:privateScene}=await import(pathToFileURL(path.join(privateBase,'visuals/dantas2002-protocol.mjs')).href+'?audit='+Date.now());
...
for(const rid of allRecordIds){
 ...
 for(const op of record.operations){
  operationCount++;const scene=buildDantas2002Scene(op,record),expected=privateScene(op,record);
...
check('All 48 operations have source-specific scenes',operationCount===48);
...
for(const rid of allRecordIds){
 ...
 for(const [index,op] of record.operations.entries()){
  ...
  const tree=descend(host),copy=tree.find(x=>x.className==='protocol-copy'),art=tree.find(x=>x.className==='protocol-art protocol-art-dantas2002');
  check(rid+'/'+op.id+' mounted source-specific artwork',art?.innerHTML===buildDantas2002Scene(op,record).svg);
...
check('All 48 actual mounted stages exercised',mountedOperations===48);
check('All 67 actual condition quantities displayed',mountedQuantities===67);
const failures=checks.filter(c=>!c.passed);
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled DOM execution of actual source-reader and apparatus modules, exact original-asset accessibility and source-defined molecular bindings. No browser geometry or publication claim.',reader_items:cards.length,unique_original_assets:filenames.length,operation_count:operationCount,mounted_operations:mountedOperations,mounted_quantities:mountedQuantities,binding_count:bindingCount,checks_passed:checks.length-failures.length,check_count:checks.length,failures,console_errors:consoleErrors,checks,artifact_sha256:Object.fromEntries(['paper-review.mjs','source-evidence.mjs','protocol-visuals.mjs','quantity-value.mjs','dantas2002-protocol.mjs','data/paper-reviews/dantas2002.json','assets/chemical-registry/bindings.json','assets/chemical-registry/registry.json'].map(p=>['dist/'+p,digest(path.join(dist,p))]))};
