...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='dantas2002'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/dantas2002.json');reader=read(S/'data/paper-reviews/dantas2002.json');generated=read(S/'dist/data/paper-reviews/dantas2002.json')
 ...
 ck('Independent reader audit binds current candidate',reader_science['reader_sha256']==sha(B/'public-review-proposal/dantas2002.json') and reader_science['record_hashes']==science['record_hashes'])
 ...
 ck('16 records / 48 operations / 110 measurements',len(records)==16 and sum(len(r['operations']) for r in records.values())==48 and sum(len(r['measurements']) for r in records.values())==110)
 ck('6 literature protocols / 8 procedures / 2 observations',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=6,procedure=8,observation=2))
 ...
 ck('Electronic model threshold stays under Properties',not structural('source_strong_confinement_size_upper_bound'))
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=dantas2002' in raw)
  ...
  for n,m in enumerate(r['measurements']):
   ...
   if m['property'] in expected_structure or m['property']=='source_strong_confinement_size_upper_bound':
 ...
 ck('101 reader items intact',set(items)==set(pi) and len(items)==101)
 ...
 ck('147 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==147)
 ...
 ck('11 original assets and category counts',set(assets)==set(crops) and len(assets)==11 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[7,0,0,4])
 ...
 ck('Figure 4 AFM cohort association',assets['figure-4']['sample_links']==['dantas-2002-afm1','dantas-2002-afm2','dantas-2002-afm-analysis'])
 ck('Figure 6 SG1 association',assets['figure-6']['sample_links']==['dantas-2002-sg1','dantas-2002-power-response','dantas-2002-mechanisms'])
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==101 and runtime['unique_original_assets']==11 and runtime['operation_count']==48 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/dantas2002.json',S/'dist/data/paper-reviews/dantas2002.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'data/measurement-display.json',S/'dist/data/measurement-display.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/dantas2002-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/crystal-viewer.mjs',S/'dist/material-hub.mjs',S/'dist/assets/chemical-registry/product-bindings.json',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/dantas2002.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 11 originals, material hubs, 48 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':16,'operations':48,'measurements':110,'reader_items':101,'source_units':147,'original_assets':11,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Dantas 2002 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
