...
ledger=read(MON/'ledger.json');assert ledger['current_paper']['group_id']=='10.1021_cm0115416'
g=ledger['groups']['10.1021_cm0115416'];fp=g['fingerprint']
assert fp['generation']==g['generation']==2
assert fp['bundle_sha256']=='5025768ecf6ddd21d556ffb63dac2e912982fc4adf6835cdd0b9bebc647e56e7'
assert set(fp['files'].values())=={'6438ee53b3fffb86f5e36508f265041d9235b8ee9f3e20dd99d46aae91c68a57'}
write(B/'final-source-fingerprint.json',{'group_id':'10.1021_cm0115416',**fp})
inv=read(S/'data/inventory-summary.json');row=next(x for x in inv['per_paper']if x['source_group']=='yi2002')
index=read(S/'dist/data/materials-index.json');hub=next(x for x in index['materials']if x['formula']=='La2(MoO4)3:Yb,Er')
reader=read(S/'data/paper-reviews/yi2002.json');audit=read(B/'source-audit.json');records=[read(S/'data/records'/(k+'.json'))for k in row['record_ids']]
archive=S/'.sites-runtime/site-yi2002.tar.gz';assert sha(archive)==read(B/'archive-validation.json')['sha256']
p=dict(status='published',source_commit=saved['source']['commit_sha'],site_project_id=saved['project_id'],public_live_version=saved['version_number'],dataset_version='0.18.0',source_pushed=True,archive=str(archive.relative_to(M)),archive_sha256=sha(archive),archive_storage=saved['archive_storage'],version_id=saved['id'],deployment_id=d['id'],deployment_status=d['status'],public_url=d['url'],published_at=d['updated_at'],review_scope='supplied_main_only_si_unverified',source_id='yi2002',doi='10.1021/cm0115416',records=len(records),synthesis_routes=row['synthesis_route_variant_count'],controls=row['contextual_control_count'],supporting_procedures=row['procedure_count'],contextual_observations=row['contextual_observation_count'],measurement_entries=sum(len(r['measurements'])for r in records),operations=sum(len(r['operations'])for r in records),reader_evidence_items=sum(len(s['items'])for s in reader['reader_sections']),source_audit_units=audit['unit_count'],typed_facts=sum(reader['counts'][k]for k in ['typed_characterization_rows','operation_parameter_facts','reagent_quantity_facts','stock_quantity_facts']),source_typed_anchors=audit['typed_anchor_count'],original_assets=len(read(B/'reader-assets/crop-manifest.json')['assets']),original_figures=10,original_tables=0,canonical_records_total=inv['summary']['canonical_records'],material_hubs_total=len(index['materials']),source_groups_total=inv['summary']['total_canonical_source_groups'],source_documents_unchanged=True,source_generation=2,source_fingerprint_at=fp['created_at'],bundle_sha256=fp['bundle_sha256'],native_publication_evidence='native-publication.json',browser_checks='browser-qa.json',build_checks='build-validation.json',independent_audit=['source-audit.json','canonical-records-audit.json','reader-source-audit.json','molecular-source-audit.json','bindings-source-audit.json','visual-source-audit.json','public-review-proposal/proposal-validation.json','reader-assets/integrated-presentation-audit.json'],material_url=d['url'].rstrip('/')+'/'+hub['url'],paper_url=d['url'].rstrip('/')+'/paper-review.html?id=yi2002',si_status='No SI declaration observed in supplied main; matching SI not located or verified.')
assert len(records)==18 and p['synthesis_routes']==6 and p['operations']==105
