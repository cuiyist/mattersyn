...
r=read(O/'yi2002.json');drafts={p.stem:read(p) for p in (B/'canonical-drafts').glob('*.json')};manifest=read(B/'reader-assets/crop-manifest.json');source=read(B/'source-manifest.json');audit=read(B/'source-audit.json');checks=[]
...
ck('all reference entries',[x['reference_number'] for x in r['referenced_methods']],list(range(1,28)));ck('all figures',sorted(a['id'] for a in r['figures']),sorted('figure-'+str(n) for n in range(1,11)))
...
ck('Figure 1 explicit 800 C cohort',assets['figure-1']['sample_links'],['yi-2002-anneal-800','yi-2002-xrd'])
