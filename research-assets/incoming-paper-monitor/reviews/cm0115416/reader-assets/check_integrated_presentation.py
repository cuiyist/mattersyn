...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='yi2002'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/yi2002.json');reader=read(S/'data/paper-reviews/yi2002.json');generated=read(S/'dist/data/paper-reviews/yi2002.json')
 ...
 ck('Independent reader audit binds current candidate',reader_science['reader_sha256']==sha(B/'public-review-proposal/yi2002.json') and reader_science['record_hashes']==science['record_hashes'])
 ...
 ck('18 records / 105 operations / 91 measurements',len(records)==18 and sum(len(r['operations']) for r in records.values())==105 and sum(len(r['measurements']) for r in records.values())==91)
 ck('6 literature protocols / 11 procedures / 1 observation',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=6,procedure=11,observation=1))
 ...
 expected_structure={'instrumental_average_particle_diameter', 'tem_majority_particle_diameter_range', 'tem_morphology', 'scherrer_crystallite_size', 'tem_scale_bar', 'annealing_morphology_observation', 'author_particle_crystallinity_inference', 'phase_purity_limit', 'figure3_distribution_scope', 'instrumental_majority_particle_diameter_range', 'abstract_summary_particle_diameter', 'annealing_size_comparison', 'author_size_method_agreement', 'figure1_display_scope', 'diffraction_reference'}
 ...
 expected_optical={'prose_power_law_exponent','figure_power_law_exponent','author_inferred_excitation_photon_count'}
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=yi2002' in raw)
 ...
 ck('115 reader items intact',set(items)==set(pi) and len(items)==115)
 ...
 ck('148 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==148)
 ...
 ck('16 original assets and category counts',set(assets)==set(crops) and len(assets)==16 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[10,0,0,6])
 ...
 ck('Figure 1 explicit 800 C association',assets['figure-1']['sample_links']==['yi-2002-anneal-800','yi-2002-xrd'])
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==115 and runtime['unique_original_assets']==16 and runtime['operation_count']==105 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/yi2002.json',S/'dist/data/paper-reviews/yi2002.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'data/measurement-display.json',S/'dist/data/measurement-display.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/yi2002-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/crystal-viewer.mjs',S/'dist/material-hub.mjs',S/'dist/assets/chemical-registry/product-bindings.json',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/yi2002.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 16 originals, material hubs, 105 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':18,'operations':105,'measurements':91,'reader_items':115,'source_units':148,'original_assets':16,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Yi 2002 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
