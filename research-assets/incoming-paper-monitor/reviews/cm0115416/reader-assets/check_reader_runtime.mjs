...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/yi2002.json'),'utf8'));
...
globalThis.location={search:'?id=yi2002',hash:''};
...
check('All 115 source items executed',cards.length===115&&items.length===115);
...
check('Ten original figure cards',figureCards.length===10);
...
check('All 16 original assets accessible',allAssets.length===16&&filenames.length===16&&allAssets.every(a=>filenames.includes(a.public_asset.split('/').pop())));
...
check('SI limitation explicit',cardMap.get('coverage')?.textContent.includes('SI'));
...
if(input?.events.input){input.value='Scherrer';input.events.input();check('Source search filters meaningful subset',cards.some(n=>!n.hidden)&&cards.some(n=>n.hidden));input.value='';input.events.input();check('Cleared search restores all items',cards.every(n=>!n.hidden));}else check('Source search wired',false);
...
const {buildYi2002Scene}=await import(pathToFileURL(path.join(dist,'yi2002-protocol.mjs')).href+'?audit='+Date.now());
const {buildYi2002Scene:privateScene}=await import(pathToFileURL(path.join(privateBase,'visuals/yi2002-protocol.mjs')).href+'?audit='+Date.now());
...
for(const rid of allRecordIds){
 ...
 for(const op of record.operations){
  operationCount++;const scene=buildYi2002Scene(op,record),expected=privateScene(op,record);
...
check('All 105 operations have source-specific scenes',operationCount===105);
...
for(const rid of allRecordIds){
 ...
 for(const [index,op] of record.operations.entries()){
  ...
  const tree=descend(host),copy=tree.find(x=>x.className==='protocol-copy'),art=tree.find(x=>x.className==='protocol-art protocol-art-yi2002');
  check(rid+'/'+op.id+' mounted source-specific artwork',art?.innerHTML===buildYi2002Scene(op,record).svg);
...
check('All 105 actual mounted stages exercised',mountedOperations===105);
check('All 138 actual condition quantities displayed',mountedQuantities===138);
const failures=checks.filter(c=>!c.passed);
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled DOM execution of actual source-reader and apparatus modules, exact original-asset accessibility and source-defined molecular bindings. No browser geometry or publication claim.',reader_items:cards.length,unique_original_assets:filenames.length,operation_count:operationCount,mounted_operations:mountedOperations,mounted_quantities:mountedQuantities,binding_count:bindingCount,checks_passed:checks.length-failures.length,check_count:checks.length,failures,console_errors:consoleErrors,checks,artifact_sha256:Object.fromEntries(['paper-review.mjs','source-evidence.mjs','protocol-visuals.mjs','quantity-value.mjs','yi2002-protocol.mjs','data/paper-reviews/yi2002.json','assets/chemical-registry/bindings.json','assets/chemical-registry/registry.json'].map(p=>['dist/'+p,digest(path.join(dist,p))]))};
