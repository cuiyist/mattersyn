...
inv=read(S/'data/inventory-summary.json');row=next(x for x in inv['per_paper']if x['source_group']=='yi2002');records=[read(S/'data/records'/(k+'.json'))for k in row['record_ids']]
assert len(records)==18 and sum(len(r['operations'])for r in records)==105 and sum(len(r['measurements'])for r in records)==91
m=read(B/'milestones.json')
m['extract']={'status':'complete','evidence':[str(B/f)for f in ['canonical-record-manifest.json','public-review-proposal/proposal-validation.json','reader-assets/crop-manifest.json']],'note':'Six synthesis routes, eleven supporting procedures and one interpretation observation retain 105 operations and 91 measurement entries. All 148 source units mapped to the 115-item reader. Matching SI unverified.'}
m['audit']={'status':'complete','evidence':[str(B/f)for f in audits],'note':'Independent source, canonical, reader, sixteen original assets, molecular, binding and 105 apparatus-scene audits passed. Stock mass/mole and feed/formula conflicts, comparison-batch missingness, spectral label discrepancies and separate analytical specimen identities remain explicit.'}
m['integrate']={'status':'complete','evidence':[str(B/f)for f in ['integration-manifest.json','build-validation.json','browser-qa.json','reader-assets/integrated-presentation-audit.json','reader-assets/reader-runtime-check.json']],'note':f"Integrated into the same existing MatterSyn atlas with {inv['summary']['canonical_records']} canonical records. One new lanthanum-molybdate material hub, six equal route cards, periodic discovery and all original figures. Build, independent runtime checks and representative browser inspection passed."}
