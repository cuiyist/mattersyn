...
check(len(new)==17 and not set(new)&set(old),'Seventeen unique new identities; no overwrite or duplicate existing ID')
expected={'octane':{'C':8,'H':18}}
images=[]
for ident,e in new.items():
    check(e['pubchemCid'] is None,ident+' no invented external identifier')
    check(e['sourceUrls']==['https://doi.org/10.1021/jp971091y'],ident+' source identity provenance')
...
check(new['identity-dabbousi-rhodamine590']['formula'] is None and new['identity-dabbousi-rhodamine640']['formula'] is None,'Dye standard identities remain unresolved')
