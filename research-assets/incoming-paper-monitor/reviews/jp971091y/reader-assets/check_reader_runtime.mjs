...
const ledger=JSON.parse(fs.readFileSync(path.join(dist,'data/paper-reviews/dabbousi1997.json'),'utf8'));
...
globalThis.location={search:'?id=dabbousi1997',hash:''};
...
check('16 original figure cards',figCards.length===16);
...
check('25 unique original assets accessible',filenames.length===25);
const allText=document.getElementById('review-body').textContent;
check('Table1 measured major axis and percentage spread render',allText.includes('tem major axis A: 39')&&allText.includes('tem relative spread percent: 8.2'));
check('Original source note22 reader link visible',cardMap.get('size-temperature-pairs').textContent.includes('Original source note 22')&&filenames.includes('source-note-22.png'));
...
const result={status:failures.length?'failed':'passed',checked_utc:new Date().toISOString(),scope:'Controlled local DOM execution of actual source reader modules; excludes browser geometry, interactions and apparatus visual acceptance.',reader_items:cards.length,original_asset_links:assetlinks.length,unique_original_assets:filenames.length,original_asset_filenames:filenames,checks_passed:checks.length-failures.length,check_count:checks.length,failures,checks,artifact_sha256:{'dist/paper-review.mjs':digest(path.join(dist,'paper-review.mjs')),'dist/source-evidence.mjs':digest(path.join(dist,'source-evidence.mjs')),'dist/data/paper-reviews/dabbousi1997.json':digest(path.join(dist,'data/paper-reviews/dabbousi1997.json'))}};
