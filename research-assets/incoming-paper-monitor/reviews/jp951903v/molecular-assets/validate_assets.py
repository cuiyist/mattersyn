...
check(len(new)==10 and not set(new)&set(old),'Ten unique new identities; no overwrite or duplicate existing ID')
expected={'methyl-isobutyl-ketone':{'C':6,'H':12,'O':1},'tetrafluoromethane':{'C':1,'F':4},'trifluoromethane':{'C':1,'H':1,'F':3},'hydrogen-fluoride':{'H':1,'F':1},'germane':{'Ge':1,'H':4}}
images=[]
for ident,e in new.items():
    check(e['pubchemCid'] is None,ident+' no invented external identifier')
    check(e['sourceUrls']==['https://doi.org/10.1021/jp951903v'],ident+' source identity provenance')
    ...
    for key in ['model2dPath','model3dPath']:
        ...
        if key=='model2dPath':
            ...
            check(m['conformerGeneration']['minimizationReturnCode']==0,ident+' classical optimization convergence')
            ...
            center={'germane':'Ge','tetrafluoromethane':'C','trifluoromethane':'C'}.get(ident)
            if center:
                atom=[a for a in aa if a['element']==center][0]
                check(sum(b['a']==atom['index'] or b['b']==atom['index'] for b in bb)==4,ident+' four central-atom bonds')
...
check(new['identity-silicon-nitride-afm-tip']['formula'] is None,'Unspecified silicon nitride stoichiometry preserved')
check(not new['identity-pmma-resist']['model3dPath'],'No invented polymer chain')
check('identity-carbon-whisker-afm-tip'!=bindings['recordBindings']['heath-1996-ge-characterization'].get('silicon-nitride-tip'),'Different AFM equipment components not collapsed')
width=1400;height=5*340;sheet=Image.new('RGB',(width,height),'white');draw=ImageDraw.Draw(sheet)
for n,(ident,im) in enumerate(images):
    x=(n%2)*700;y=(n//2)*340;draw.text((x+15,y+10),ident,fill='black');sheet.paste(im,(x+14,y+40))
sheet.save(OUT/'review/svg-contact-sheet.png')
...
print(json.dumps({'status':report['status'],'checks':len(checks),'errors':fail,'contactSheet':str(OUT/'review/svg-contact-sheet.png')}))
assert not fail
