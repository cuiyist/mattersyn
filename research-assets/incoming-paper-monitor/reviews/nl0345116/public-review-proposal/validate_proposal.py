...
r=read(O/'sashchiuk2004.json');drafts={p.stem:read(p) for p in (B/'canonical-drafts').glob('*.json')};manifest=read(B/'reader-assets/crop-manifest.json');source=read(B/'source-manifest.json');audit=read(B/'source-audit.json');checks=[]
...
ck('Supplied main page count',[sources[k]['page_count'] for k in sources],[7]);ck('SI not silently verified',r['supporting_information']['status'],'not_located')
...
ck('all reference entries',[x['reference_number'] for x in r['referenced_methods']],list(range(1,68)));ck('all figures',sorted(a['id'] for a in r['figures']),['figure-'+str(n) for n in range(1,6)])
...
ck('Figure1 individual and sphere contexts',assets['figure-1']['sample_links'],['sashchiuk-2004-individual-structure','sashchiuk-2004-sphere-structure'])
