...
for(const name of fs.readdirSync(path.join(root,'canonical-drafts')).filter(s=>s.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){const s=buildYaoScene(o,r);check(!!s,r.record_id+'/'+o.id+' covered');if(!s)continue;
  check(!/undefined|NaN|null|<script|onload=/.test(s.svg),r.record_id+'/'+o.id+' safe finite SVG');
  check(buildYaoScene(o,{...r,sources:[{doi:'10.1021/not-this-paper'}]})===null,r.record_id+'/'+o.id+' wrong DOI rejected');
  check(buildYaoScene(o,{...r,record_id:'another-paper'})===null,r.record_id+'/'+o.id+' wrong record family rejected');
  check(buildYaoScene({...o,action:'unknown'},r)===null,r.record_id+'/'+o.id+' unknown action rejected');
...
check(rows.length===47,'Exactly current 47 canonical operations');check(new Set(rows.map(x=>x.action)).size===25,'Exactly 25 distinct source actions');
fs.writeFileSync(path.join(out,'scene-manifest.json'),JSON.stringify({module_sha256:sha(fs.readFileSync(path.join(root,'yao-protocol.mjs'))),source_doi:'10.1021/la970480g',illustrative_only:true,eligible_training:false,rows},null,2));
