...
B=Path(__file__).resolve().parent;S=B.parents[3]/'recipe-atlas';p=S/'.sites-runtime/site-v21.tar.gz'
with tarfile.open(p,'r:gz') as t:
 ...
 assert 'dist/data/paper-reviews/gerion2001.json'in names and 'dist/gerion2001-protocol.mjs'in names
 assert not any('/.git/'in x or x.startswith('.git/')for x in names)
v={'status':'passed','archive':str(p),'size_bytes':p.stat().st_size,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'entry_count':len(names),'source_commit':'ccc6edea9ba91b45f129925d9f09a663881232ab'}
