...
r=read(O/'gerion2001.json');drafts={p.stem:read(p) for p in (B/'canonical-drafts').glob('*.json')};manifest=read(B/'reader-assets/crop-manifest.json');source=read(B/'source-manifest.json');audit=read(B/'source-audit.json');checks=[]
...
ck('source hash',sha(Path(source['source_path'])),source['sha256']);ck('main page count',source['main_page_count'],11)
...
ck('all reference entries',[x['reference_number'] for x in r['referenced_methods']],list(range(1,37)));ck('all figures',sorted(a['id'] for a in r['figures']),sorted('figure-'+str(n) for n in range(1,8)))
...
ck('Figure 2 optical scope',assets['figure-2']['sample_links'],['gerion-2001-optical-properties'])
ck('Figure 5 yellow scope',assets['figure-5']['sample_links'],['gerion-2001-size-yellow'])
ck('Figure 6 green HPLC scope',assets['figure-6']['sample_links'],['gerion-2001-hplc-acquisition','gerion-2001-hplc-purification'])
ck('Note 34 scattering context',assets['note-34']['sample_links'],['gerion-2001-upstream-controls'])
for row in assets['table-1']['structured_rows']:
 mm={m['id']:m for m in drafts[row['canonical_record_id']]['measurements']};c=row['color']
 for field,suffix in [('absorption_nm','absorption'),('emission_nm','emission'),('quantum_yield_percent','qy'),('relative_quantum_yield_percent','relative-qy' if c=='blue' else 'retention')]:
  if c=='blue' and suffix=='relative-qy' and c+'-'+suffix not in mm:continue
  ck('Table1 '+c+' '+field,row[field],mm[c+'-'+suffix]['value']['value'])
for row in assets['table-2']['structured_rows']:
 mm={m['id']:m for m in drafts[row['canonical_record_id']]['measurements']}
 for field,mid in [('core_tem_nm','core-diameter'),('core_shell_tem_nm','core-shell-diameter'),('silica_afm_nm','silica-height'),('mpa_afm_nm','mpa-height')]:ck('Table2 '+row['color']+' '+field,row[field],mm[mid]['value']['value'])
result={'status':'passed' if all(c['passed'] for c in checks) else 'failed','checked_at':datetime.now(timezone.utc).isoformat(),'scope':'Private canonical, reader, source-unit and asset integrity; independent science/crop audit and integrated browser checks remain separate.','check_count':len(checks),'failed_checks':[c for c in checks if not c['passed']],'counts':{'items':len(items),'source_units':len(audit['units']),'records':len(drafts),'operations':len(ops),'measurements':len(meas),'typed_facts':len(quantities),'assets':len(assets)},'hashes':{'reader':sha(O/'gerion2001.json'),'source_audit':sha(B/'source-audit.json'),'crop_manifest':sha(B/'reader-assets/crop-manifest.json'),'canonical':{rid:sha(B/'canonical-drafts'/f'{rid}.json') for rid in drafts}},'checks':checks}
