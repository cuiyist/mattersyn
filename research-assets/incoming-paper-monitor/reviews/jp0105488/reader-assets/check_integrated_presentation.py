...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='gerion2001'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/gerion2001.json');reader=read(S/'data/paper-reviews/gerion2001.json');generated=read(S/'dist/data/paper-reviews/gerion2001.json')
 ...
 ck('28 records / 82 operations / 186 measurements',len(records)==28 and sum(len(r['operations']) for r in records.values())==82 and sum(len(r['measurements']) for r in records.values())==186)
 ck('2 routes / 16 procedures / 10 observations',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=2,procedure=16,observation=10))
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=gerion2001' in raw)
 ...
 ck('139 reader items intact',set(items)==set(pi) and len(items)==139)
 ...
 ck('251 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==251)
 ...
 ck('17 original assets and category counts',set(assets)==set(crops) and len(assets)==17 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[7,2,0,8])
 ...
 ck('Optical Figure 2 retains optical-only scope',assets['figure-2']['sample_links']==['gerion-2001-optical-properties'])
 ck('Figure 5 retains common yellow size scope',assets['figure-5']['sample_links']==['gerion-2001-size-yellow'])
 ck('Missing SI stays explicit',reader['supporting_information']['status']=='declared_not_located_or_matched' and not any(d['role']=='si' for d in reader['documents']))
 ...
 for formula,expected_evidence in reader['material_evidence_records'].items():
  ...
  if not entries:
   ck(formula+' absent component-only hub permitted',not any(r['material']['formula']==formula and r['record_type']=='literature_protocol' for r in records.values()));continue
  entry=entries[0];hp=S/'dist/data/materials'/f"{entry['id']}.json";hub=read(hp);bind(hp)
  expected={rid for rid,r in records.items() if r['record_type']=='literature_protocol' and (r['material']['formula']==formula or formula in r['material'].get('components',[]))}
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==139 and runtime['unique_original_assets']==17 and runtime['operation_count']==82 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/gerion2001.json',S/'dist/data/paper-reviews/gerion2001.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/gerion2001-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/gerion2001.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 17 originals, material hubs, 82 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':28,'operations':82,'measurements':186,'reader_items':139,'source_units':251,'original_assets':17,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Gerion 2001 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
