...
r=read(O/'schwartz2003.json');drafts={p.stem:read(p) for p in (B/'canonical-drafts').glob('*.json')};manifest=read(B/'reader-assets/crop-manifest.json');source=read(B/'source-manifest.json');audit=read(B/'source-audit.json');checks=[]
...
sources={d['role_candidate']:d for d in source['documents'] if d['role_candidate'] in {'main','si'}}
for role,d in sources.items():ck(role+' source hash',sha(Path(d['path'])),d['sha256'])
ck('main/SI page count',[sources[k]['page_count'] for k in ['main','si']],[14,4])
...
ck('all reference entries',[x['reference_number'] for x in r['referenced_methods']],list(range(1,52)));ck('all figures',sorted(a['id'] for a in r['figures']),sorted(['figure-'+str(n) for n in range(1,12)]+['si-figure-'+str(n) for n in range(1,7)]))
