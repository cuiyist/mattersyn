...
import {buildSchwartz2003Scene} from './schwartz2003-protocol.mjs';
...
for(const file of fs.readdirSync(path.join(B,'canonical-drafts')).filter(n=>n.endsWith('.json')).sort()){
 ...
 for(const o of r.operations){
  const before=JSON.stringify({o,r}),s=buildSchwartz2003Scene(o,r),key=r.record_id+'/'+o.id;
...
const out={module:'schwartz2003-protocol.mjs',module_sha256:sha(fs.readFileSync(path.join(V,'schwartz2003-protocol.mjs'))),record_count:30,operation_count:scenes.length,checks,scenes};
