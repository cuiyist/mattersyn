...
O=Path(__file__).resolve().parent;B=O.parent;S=B.parents[3]/'recipe-atlas';SID='schwartz2003'
...
def main():
 ...
 proposal=read(B/'public-review-proposal/schwartz2003.json');reader=read(S/'data/paper-reviews/schwartz2003.json');generated=read(S/'dist/data/paper-reviews/schwartz2003.json')
 ...
 ck('Independent reader audit binds current candidate',reader_science['reader_sha256']==sha(B/'public-review-proposal/schwartz2003.json') and reader_science['canonical_record_hashes']==science['record_hashes'])
 ...
 ck('30 records / 95 operations / 277 measurements',len(records)==30 and sum(len(r['operations']) for r in records.values())==95 and sum(len(r['measurements']) for r in records.values())==277)
 ck('3 literature protocols / 20 procedures / 7 observations',Counter(r['record_type'] for r in records.values())==Counter(literature_protocol=3,procedure=20,observation=7))
 ...
 expected_structure={'aggregation_diffraction_comparison', 'reported_specimen_dopant_percentage', 'particle_histogram_population', 'optically_estimated_particle_diameter', 'cited_zno_lattice_parameter_c', 'reported_specimen_cobalt_percentage', 'scherrer_comparison_scope', 'wide_area_diffraction_assignment', 'si6_reported_particle_diameter', 'tem_mean_particle_diameter', 'micrograph_scale_bar', 'atomic_structure_availability', 'dopant_measurement_scope', 'si5_reported_dopant_percentage', 'overview_visible_particle_count', 'cited_zno_lattice_parameter_a', 'powder_xrd_phase_assignment', 'tem_reported_distribution_width', 'aggregate_morphology', 'reported_optical_specimen_diameter', 'reported_starting_dot_diameter', 'aggregation_xrd_comparison', 'si_nickel_microscopy_scope', 'sem_scale_bar'}
 ...
 expected_optical={'mcd_c0_d0','mcd_b0_d0','mcd_monitor_energy','table1_mcd_transition_energy','average_zeeman_redshift_per_tesla','curie_temperature_lower_bound','fitted_n0_beta','estimated_n0_beta','room_temperature_coercivity','trap_luminescence_quenching_fraction'}
 ...
 for rid,r in records.items():
  ...
  ck(rid+' record HTML links to source reader','paper-review.html?id=schwartz2003' in raw)
 ...
 ck('184 reader items intact',set(items)==set(pi) and len(items)==184)
 ...
 ck('261 source units retained',set(sourcecoverage['unit_to_reader_items'])=={u['id'] for u in source['units']} and len(source['units'])==261)
 ...
 ck('38 original assets and category counts',set(assets)==set(crops) and len(assets)==38 and [len(reader[k]) for k in ['figures','tables','equations','source_notes']]==[17,2,10,8])
 ...
 ck('Figure 5 separately scoped microscopy',assets['figure-5']['sample_links']==['schwartz-2003-microscopy'])
 ...
 runtime=read(O/'reader-runtime-check.json');ck('Actual source-reader/apparatus execution passed',runtime['status']=='passed' and runtime['reader_items']==184 and runtime['unique_original_assets']==38 and runtime['operation_count']==95 and runtime['binding_count']==n)
 ck('Runtime hashes are current',all(sha(S/p)==h for p,h in runtime['artifact_sha256'].items()))
 for p in [S/'data/paper-reviews/schwartz2003.json',S/'dist/data/paper-reviews/schwartz2003.json',S/'dist/data/dataset-manifest.json',S/'dist/data/materials-index.json',S/'data/measurement-display.json',S/'dist/data/measurement-display.json',S/'dist/data/records.jsonl',S/'dist/paper-review.mjs',S/'dist/source-evidence.mjs',S/'dist/schwartz2003-protocol.mjs',S/'dist/protocol-visuals.mjs',S/'dist/crystal-viewer.mjs',S/'dist/material-hub.mjs',S/'dist/assets/chemical-registry/product-bindings.json',S/'dist/assets/chemical-registry/bindings.json',S/'dist/assets/chemical-registry/registry.json',B/'canonical-records-audit.json',B/'source-audit.json',B/'public-review-proposal/schwartz2003.json',O/'reader-runtime-check.json']:bind(p)
 failures=[c for c in checks if not c['passed']]
 result={'status':'passed' if not failures else 'findings','checked_utc':datetime.now(timezone.utc).isoformat(),'scope':'Actual integrated records, generated record HTML, source reader, 38 originals, material hubs, 95 operation scenes and source-defined bindings. Browser geometry and publication are separate.','counts':{'records':30,'operations':95,'measurements':277,'reader_items':184,'source_units':261,'original_assets':38,'material_bindings':n},'check_count':len(checks),'checks_passed':len(checks)-len(failures),'findings':failures,'promotion_differences':promotions,'artifact_sha256':hashes,'checks':checks}
 (O/'integrated-presentation-audit.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf8')
 (O/'integrated-presentation-audit.md').write_text('# Schwartz 2003 integrated presentation check\n\n'+result['status']+f"; {result['checks_passed']}/{len(checks)} checks.\n\n"+result['scope']+'\n\nFindings: '+json.dumps(failures,ensure_ascii=False)+'\n',encoding='utf8')
