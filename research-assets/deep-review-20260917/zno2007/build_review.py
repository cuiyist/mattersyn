...
e=E(EXP)
base=json.loads((OUT/'s1-base-snapshot.json').read_text(encoding='utf-8'))
base['revision']=4
...
add_m(base,'s1-ftir-c-c','fu2007-s1','ftir_band',1640,'cm^-1','ATR-FTIR','Main pp.16030–16031, Fig.3 upper S1 spectrum','Authors assign C=C stretching in OA; dried purified particles.')
add_range(base,'s1-ftir-interface','fu2007-s1','ftir_band_region',1400,1600,'cm^-1','Main pp.16030–16031, Fig.3 upper S1 spectrum','Two bands; authors assign C=O stretching in ZnO/OA surface complexes. Exact individual positions not supplied.')
add_range(base,'s1-ftir-nh-oh','fu2007-s1','ftir_band_region',3300,3600,'cm^-1','Main pp.16030–16031, Fig.3 upper S1 spectrum','Authors assign DEA N–H and surface O–H; not quantitative ligand coverage.')
...
records=[base]
def mark_preparation_inherited(obj,label):
...
def variant(label,title):
    r=copy.deepcopy(base);r['record_id']='fu-2007-zno-'+label.lower();r['revision']=2;r['record_type']='protocol_variant';r['title']=title
    r['lineage']['parent_record_id']=base['record_id'];r['measurements']=[];r['quality']['conflicts']=[]
    for key in ['materials','stocks','operations']:mark_preparation_inherited(r[key],label)
...
comb['description']='Combine the two aqueous stocks without oleic acid, as specified for S2. Unchanged stock charges are inherited from S1. The calculated 100 mL water-charge sum is not a measured final volume; order and addition rates are unspecified.'
...
s2['products'][0]['surface']=fact('DEA and hydroxyl association inferred by authors from FTIR',E('Main pp.16030–16031, Fig.3 lower S2 spectrum'),status='author_derived')
...
s2['quality']['conflicts']=copy.deepcopy(base['quality']['conflicts'])
...
for mid,lo,hi,assign in [('cncoh',1000,1100,'C–N and C–OH'),('nh',1600,1650,'N–H bending'),('ch',2850,3000,'C–H'),('nhoh',3300,3600,'N–H and O–H')]:
    add_range(s2,'s2-ftir-'+mid,'fu2007-s2','ftir_band_region',lo,hi,'cm^-1','Main pp.16030–16031, Fig.3 lower S2 spectrum','Authors’ assignments: '+assign+'.')
...
s4['operations'][1]['description']='Prepare 4.0 mM aqueous zinc nitrate for S4. The 50 mL distilled-water charge is inherited from the unchanged common framework; the S4 salt mass and measured final stock volume are not reported.'
...
add_m(s4,'s4-pl-reduction','fu2007-s4','relative_photoluminescence_intensity_reduction',28.8,'%','PL','Main p.16033 discussion; Fig.6a on p.16032','Relative to S1 under compared measurement conditions; not a QY or synthesis yield.')
records.append(s4)
for suffix,title,parent,reagent,formula,volume,times in [
 ...
 ('s1-hcl-perturbation','Dilute-HCl perturbation of S1 ZnO dispersion','S1','hydrochloric_acid','HCl',None,[1,3,8])]:
    loc='Main p.16033 treatment discussion; Fig.6'+('b' if parent=='S2' else 'c')+' on p.16032';ee=E(loc)
    r=record('fu-2007-zno-'+suffix,title,'ZnO','Metal oxides','Post-synthesis ligand/interface control',copy.deepcopy(base['sources'][0]),loc,kind='procedure');r['revision']=2;r['collection']='reviewed_literature';r['material']=copy.deepcopy(base['material']);r['lineage']['recipe_family']='fu2007-zno-post-treatment';r['lineage']['parent_record_id']='fu-2007-zno-'+parent.lower()
...
figs=[
 ...
 ('fig4','main',3,[662,82,1064,981],'a: PL/PLE of S1 and aqueous S2, with both S2 traces scaled by ten; b: photographs of S1/S2 under 350 nm UV; c: S1 photostability at 440 nm under 350 nm UV, 130 µW/cm², over 48 h.',['S1','S2'],'experimental'),
 ...
 ('surface-reaction-scheme','main',3,[114,604,560,683],'Unnumbered proposed surface reaction of OA with ZnO hydroxyl groups at 80 °C; x/y are schematic occupancies, not measured surface composition.',['S1'],'author_model'),
 ...
 ('figS2','si',3,[310,145,864,594],'Five-dilution relative-QY fits; ZnO slope 113.8, R² .998; quinine sulfate slope 82.6, R² .995. 76% QY calculation.',['S1','quinine sulfate reference'],'calibration'),
 ('figS3','si',4,[389,102,805,427],'S4 HRTEM, 5 nm scale bar; 4.0 mM starting zinc stock, average size 4.2 nm reported in main text.',['S4'],'experimental')]
sources={role:ROOT/'downloaded_papers'/name for role,name in [('main','10.1021_ja075604i.pdf'),('si','10.1021_ja075604i_si_1.pdf')]}
documents=[]
sections={'main':[['Abstract','Introduction'],['Experimental','Results: XRD/TEM/FTIR'],['Results: FTIR/PL/photostability','Proposed surface-reaction scheme','Absorption-onset extrapolation'],['Quantum-confinement model','Results: absorption, dry-powder PL','Comparison controls'],['Post-treatment controls','Mechanistic model','Conclusion','References']], 'si':[['Title and author identity'],['Part1 early-stage HRTEM'],['Part2 relative quantum yield'],['Part3 S4 HRTEM']]}
...
for r in records:coverage['recipe_inventory'].append({'id':r['record_id'],'label':r['title'],'record_ids':[r['record_id']],'status':'extracted_with_reported_gaps','source_locators':[EXP] if r['record_type']!='procedure' else ['Main p.16033 treatment discussion; Fig.6b/c on p.16032'],'gaps':r['quality']['missing_fields']})
...
coverage['characterization_inventory']=[
 {'technique':'XRD','samples':['S1','S2'],'details':'Freeze-dried products; Rigaku D/max2500v/pc; wurtzite JCPDS 89-1397. Broad ~25° signal attributed to glass substrate. No refined atomic coordinates or quantitative phase fraction.','source_locators':['Main p.16030 Fig.1']},
...
(OUT/'review.md').write_text('# Fu et al.2007 — full-paper review\n\nAll 5 main pages and 4 matching SI pages were read in full and visually inspected. All 7 main figures, 3 SI figures and the unnumbered proposed surface-reaction scheme are inventoried. Four synthesis conditions (S1–S4), two post-treatment series, early-stage sampling, dry-powder characterization, blank controls and relative-QY calibration remain distinct.\n\nThe main recipe has operational omissions; a fully read paper is not automatically a complete experimental SOP or training-eligible exact-structure pair. See coverage.json for all techniques, sample links, cited-method gaps and figure provenance.\n\nS3 ammonia amount is unknown; S4 zinc salt mass is unknown. S1 numeric outcomes are not inherited into variants. Early 60 C/20 min is not a 20 min isothermal hold. S2 dry-powder emission is not aqueous S2 emission. QY calibration standards do not enter the precursor list.\n',encoding='utf-8')
