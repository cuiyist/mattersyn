...
for name in sys.argv[1:]:
    ...
    def with_asset(f,i,required=False):
        f=dict(f);asset=f.get('original_crop_asset') or f.get('asset_provenance',{});crop=f.get('crop',{})
        if isinstance(asset,str):asset={'asset':asset}
        raw=asset.get('asset') or asset.get('path') or crop.get('path') or f.get('asset_path') or f.get('crop_path') or f.get('crop_asset') or f.get('original_crop')
        if not raw:
