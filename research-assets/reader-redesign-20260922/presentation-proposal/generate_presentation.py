...
for rid in ROUTES:
    ...
    records[rid]={'source_id':sid,'title':r['title'],'material_formula':r.get('material',{}).get('formula'),
      ...
      'source':source(path,''),'data_links':{'record':f'data/records/{rid}.json','record_page':f'records/{rid}/',
        'full_review':f'paper-review.html?id={sid}' if sid in REVIEWS else ('murray-1993-characterization.html' if sid=='murray1993' else 'alivisatos-2000.html' if sid=='peng2000' else None),
...
output={'schema_version':'1.0','status':'presentation_proposal_only','input_policy':'Existing reviewed site data only; no new scientific claims, sample joins, training eligibility or publication approval.',
