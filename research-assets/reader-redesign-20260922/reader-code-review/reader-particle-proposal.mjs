...
export function mountParticleContext(panel,r,presentation={}){
 ...
 function show(){const group=groups.get(choose.value)||groups.values().next().value,d=particleDescriptor(group),layout=el('div',undefined,'reader-product-layout'),copy=el('div');
  ...
  const dl=el('dl',undefined,'reader-product-facts');for(const f of group.facts){const row=el('div');row.append(el('dt',f.label),el('dd',text(f.value)));if(f.scope)row.append(el('small',f.scope));if(f.qualifier)row.append(el('small',f.qualifier));dl.append(row);}
  ...
  copy.append(dl,link('Complete specimen evidence →',recordURL(group.recordId)+'#structures','reader-data-link'));
