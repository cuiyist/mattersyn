...
let records=[],page=0;const pageSize=12;
...
function render(){
 ...
 const matched=records.filter(r=>(!query||[r.title,r.formula,r.method,r.source_doi].join(' ').toLowerCase().includes(query))&&(!family||r.family===family)&&(!task||r.eligibility[task]?.eligible)&&(type==='all'||(type==='recipes'&&r.record_type!=='procedure')||(type==='procedure'&&r.record_type==='procedure')||(type==='reviewed'&&r.collection!=='published_benchmark'&&r.record_type!=='procedure')||(type==='benchmark'&&r.collection==='published_benchmark')));
 ...
 byId('record-count').textContent=matched.length+' matching records';byId('page-label').textContent='Page '+(page+1)+' of '+pages;byId('page-prev').disabled=page===0;byId('page-next').disabled=page+1>=pages;
}
try{
