...
function render(){
 ...
 const host=$('material-results');host.replaceChildren();$('material-count').textContent=matches.length+' material systems'+(selected.size?' containing '+[...selected].join(' + '):' in the collection');
 for(const m of matches.slice(0,60)){
  ...
  const h=document.createElement('h3');h.textContent=m.formula;const p=document.createElement('p');p.textContent=m.name;const meta=document.createElement('div');meta.className='entry-meta';meta.textContent=`${m.paper_count} indexed source associations · ${m.reviewed_records} reviewed records`+(m.benchmark_records?` · ${m.benchmark_records} benchmark rows`:'');
  const a=document.createElement('a');a.href=m.url;a.textContent='Open material →';card.append(status,h,p,meta,a);host.append(card);
 }
 if(!matches.length){const p=document.createElement('p');p.className='atlas-empty';p.textContent='No indexed material matches this combination yet. Clear an element or include indexed literature. This does not establish that the material cannot be synthesized.';host.append(p);}
...
try{const response=await fetch('data/materials-index.json');if(!response.ok)throw Error('Material index could not be loaded');const data=await response.json();materials=data.materials;
