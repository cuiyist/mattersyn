import {protocols,sampleRecords,moleculeNotes,buildCrystal} from './shape-data.mjs';
const $=id=>document.getElementById(id);
let protocolKey='typical',stepIndex=0,selectedMolecule='hpa',sampleKey='rod4b',representation='ball';
...
$('next-step').addEventListener('click',()=>showStep((stepIndex+1)%protocols[protocolKey].steps.length));showProtocol('typical');
document.querySelectorAll('[data-hpa]').forEach(button=>{button.setAttribute('aria-pressed',String(button.dataset.hpa==='5'));button.addEventListener('click',()=>{
 const value=Number(button.dataset.hpa),rod=value>=5;markButtons('[data-hpa]','hpa',button.dataset.hpa);
 $('shape-symbol').classList.toggle('rod',rod);$('hpa-outcome').textContent=rod?'Rod-like nanocrystals':'Spherical nanocrystals';
 $('hpa-caption').textContent=(rod?'Rod-like products reported':'Monodisperse spherical products reported')+' at '+value+'% HPA.';
});});
...
const planarCanvas=document.createElement('canvas');planarCanvas.id='planar-viewer';planarCanvas.className='viewer';planarCanvas.hidden=true;planarCanvas.setAttribute('aria-label','2D connectivity diagram of trioctylphosphine oxide');planarCanvas.tabIndex=0;$('molecule-viewer').after(planarCanvas);
function drawPlanar(){
...
function showMolecule(key){
 ...
 markButtons('[data-molecule]','molecule',key);$('more-reagents').value=solid||formula?key:'';
 ...
 $('molecule-badge').textContent=formula?'CONNECTIVITY SCHEMATIC':flat?'PUBCHEM 2D':solid?'SOLID REFERENCE':'PUBCHEM 3D';
 ...
 const legend=solid?[['Se','Selenium']]:formula?[]:[['C','Carbon'],['H','Hydrogen'],['P','Phosphorus'],['O','Oxygen']];
 ...
 if(formula)return;
 if(flat){planar={zoom:1,x:0,y:0,data};drawPlanar();return;}
...
$('highlight').addEventListener('change',()=>{if(selectedMolecule==='topo')drawPlanar();else showMolecule(selectedMolecule)});
...
async function initialize(){
 try{
  const results=await Promise.all(['peng2000-molecular-structures.json','peng2000-crystal-reference.json','selenium-reference.json'].map(async name=>{const r=await fetch('assets/'+name);if(!r.ok)throw Error('Structure data could not be loaded.');return r.json();}));
  ...
  showMolecule('hpa');showSample('rod4b');
