...
def main():
    ...
    assert len(reviews)==summary['formal_full_main_and_matched_si_reviews']
    assert {r['id'] for r in reviews}=={p['source_group'] for p in inventory['per_paper'] if p['review_status']=='full_supplied_main_and_matched_si_review'}, 'Inventory review scopes changed'
    assert sum(p['pages_read'] for p in reviews)==summary['formal_full_review_pages']
    ...
    rows = [('Synthesis routes and condition variants',summary['synthesis_route_variant_records'],'Source-reviewed protocol records; not necessarily complete laboratory SOPs or independent experiments.'),('Contextual controls and variants',summary['contextual_control_variant_records'],'Kept distinct from the published synthesis routes.'),('Supporting procedures',summary['shared_preparation_workup_characterization_assay_procedures'],'Precursor preparation, workup, characterization and assays.'),('Published PbS benchmark rows',summary['published_benchmark_rows'],'Numeric rows from one study, counted separately from reconstructed recipes.'),('Canonical records in total',summary['canonical_records'],'The four categories above; not a recipe or experiment count.'),('Material / component pages',summary['public_material_hubs'],'10 direct synthesis systems and 3 component-only hubs; shared records are not duplicated.'),('Complete supplied main + SI reviews',summary['formal_full_main_and_matched_si_reviews'],'74 pages across five sources. Other review scopes remain separate.'),('Paper groups without canonical records',summary['local_groups_without_canonical_records'],'Awaiting recipe verification; recipe count is unknown, not zero.')]
    ...
    statuses = {'full_supplied_main_and_matched_si_review':'Complete supplied main + SI','legacy_main_article_review_si_unverified':'Main article reviewed; SI unverified','selected_recipe_and_figure_review':'Selected recipe and figures','published_numeric_benchmark':'Numeric benchmark'}
    body += '<section class="record-section"><h2>Paper-by-paper review inventory</h2><p>The remaining local paper groups are searchable in the '+link('source library','library.html')+'. Their synthesis counts remain unknown until reviewed.</p><div class="table-scroll"><table class="reagent-table inventory-papers"><thead><tr><th>Source and review scope</th><th>Routes</th><th>Controls</th><th>Procedures</th><th>Benchmark rows</th></tr></thead><tbody>'
    for p in inventory['per_paper']:
        ...
        body += '<tr><td>'+cell+'</td>'+''.join('<td>'+str(p[k])+'</td>' for k in ['synthesis_route_variant_count','contextual_control_count','procedure_count','benchmark_row_count'])+'</tr>'
    ...
    print('Inventory reconciled: 4,176 paper groups; 10 direct systems; 32 routes; unknown full-corpus totals preserved.')
