...
def render_record(r,meta):
    ...
    for m in r['materials']:
        material_cards.append('<article class="reagent-record"><div class="record-meta"><span>'+esc(human(m['role']))+'</span><span>'+esc(human(m['stage']))+'</span></div><h3>'+esc(m['name'])+'</h3><p class="chemical-formula">'+esc(m['formula'] or 'Mixture / formula unresolved')+'</p><dl>'+quantities(m['quantities'])+'</dl><button class="molecule-link" data-molecule-name="'+esc(m['name'])+'" data-molecule-formula="'+esc(m['formula'] or '')+'" type="button">Molecular identity & structure ↗</button>'+(''.join('<p class="record-note">'+esc(n)+'</p>' for n in m['notes']))+'<small class="evidence-label">'+evidence(m['evidence'])+'</small></article>')
    ...
    if r['condition_options']:
        body+='<div class="record-notice"><strong>Alternative condition sets</strong><p>Choose one reported alternative. These are not consecutive steps or separately identified experimental batches.</p></div><div class="condition-options">'+''.join('<article><h3>'+esc(x['label'])+'</h3><dl>'+quantities(x['parameters'])+'</dl></article>' for x in r['condition_options'])+'</div>'
    ...
    body+='</div>'
    structural=[m for m in r['measurements'] if m['property'] in ['diameter','diameter_spread','particle_size','particle_size_spread','edge_length','edge_length_spread','length','width','crystal_phase']]
    ...
    optical=[m for m in r['measurements'] if m not in structural]
    s+=section('properties','04','Properties','Measurements retain their sample, technique and source',measurement_table(optical))
    links=r['context_links']
...
def main():
    ...
    manifest={'schema_version':'1.0.0','dataset_version':'0.1.0','record_count':len(records),'group_count':len(unique_groups),'families':families,'split_policy':'Connected source/recipe/parent/batch/duplicate components. Development-only below ten groups. Split thresholds are deterministic group hash 80/10/10, not a claim of balanced class coverage.','records':items}
    report={'status':'passed','dataset_version':'0.1.0','records':len(records),'recipe_records':sum(r['record_type']!='procedure' for r in records),'shared_procedures':sum(r['record_type']=='procedure' for r in records),'independent_experiment_count':None,'source_groups':len(unique_groups),'families':len(families),'eligible_by_task':{k:len(v) for k,v in exports.items()},'checks':['JSON Schema Draft 2020-12','Unique IDs and source references','Typed finite quantities and missing-value status','Operation dependencies and material-state graph','Product/measurement linkage','Illustrative-structure exclusion','Connected-component evaluation grouping','Allowlisted training inputs'],'warnings':['No source-reviewed exact experimental CIF-to-complete-recipe pair is established.','Small seed collection; no general model-performance estimate.','No experimental failure-rate or reproducibility dataset yet.']}
    report['curated_recipes']=sum(r['record_type']!='procedure' and r.get('collection')=='reviewed_literature' for r in records)
